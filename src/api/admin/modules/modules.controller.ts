...
import { CreateModuleInput, UpdateModuleInput, ModuleQueryInput } from "./modules.schema";
import { logAdminActivity } from "../../../services/admin/admin-activity.service";

// Get all modules with filtering and pagination
export const getAllModules = async (req: Request, res: Response): Promise<void> => {
  try {
    const { parentId, menuName, limit = 50, offset = 0, hierarchical = 'true' } = req.query as any;
    ...
    modules.forEach(module => {
      if (!module.parentId) {
        ...
        const menuItem = {
          ...
          submenu: moduleMap.get(module.id).submenu.map((child: any) => ({
            id: child.id,
            title: child.title,
...
export const getModuleById = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = await pool.query(
      `SELECT id, title, subtitle, path, icons, newtab, "parentId", "menuName", created_at, updated_at 
...
export const createModule = async (req: Request, res: Response): Promise<void> => {
  try {
    const moduleData: CreateModuleInput = req.body;
    ...
    const result = await pool.query(
      `INSERT INTO modules (title, subtitle, path, icons, newtab, "parentId", "menuName", created_at, updated_at)
    ...
    // Log admin activity
    await logAdminActivity(adminId, 'module_created', {
...
export const updateModule = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const updateData: UpdateModuleInput = req.body;
    const adminId = (req as any).user?.userId;
    ...
    // Log admin activity
    await logAdminActivity(adminId, 'module_updated', {
...
export const deleteModule = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Check if module exists
    const existingModule = await pool.query(
      `SELECT id, title, "menuName" FROM modules WHERE id = $1`,
      [id]
    );
    ...
    const childrenCheck = await pool.query(
      `SELECT COUNT(*) as count FROM modules WHERE "parentId" = $1`,
      [id]
    );
    ...
    // Log admin activity
    await logAdminActivity(adminId, 'module_deleted', {
...
export const getModuleHierarchy = async (req: Request, res: Response): Promise<void> => {
  try {
    const result = await pool.query(
      `SELECT 
        id, title, subtitle, path, icons, newtab, "parentId", "menuName",
