...
import { BalanceService } from "../../services/user/balance.service";
import { PromotionService } from "../../services/promotion/promotion.service";

// Get available promotions for user
export const getAvailablePromotions = async (req: Request, res: Response): Promise<void> => {
  try {
...
export const claimPromotion = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const existingClaimResult = await client.query(
      "SELECT * FROM user_promotions WHERE user_id = $1 AND promotion_id = $2",
      [userId, promotion_id]
    );
    ...
    if (promotion.type === 'welcome_bonus' || promotion.type === 'deposit_bonus' || promotion.type === 'reload_bonus') {
      ...
      const depositResult = await client.query(
        "SELECT COALESCE(SUM(amount), 0) as total_deposits FROM transactions WHERE user_id = $1 AND type = 'deposit' AND status = 'completed'",
        [userId]
      );
      ...
      bonusAmount = Math.min(
        totalDeposits * (promotion.bonus_percentage / 100),
        promotion.max_bonus_amount || Infinity
      );
    ...
    await client.query(
      `INSERT INTO user_promotions (user_id, promotion_id, status, bonus_amount, claimed_at)
       VALUES ($1, $2, 'active', $3, CURRENT_TIMESTAMP)`,
      [userId, promotion_id, bonusAmount]
    );
    ...
    if (bonusAmount > 0) {
      ...
      await client.query(
        `INSERT INTO user_balances (user_id, bonus_balance, updated_at)
         VALUES ($1, $2, CURRENT_TIMESTAMP)
         ON CONFLICT (user_id) DO UPDATE SET 
           bonus_balance = $2, updated_at = CURRENT_TIMESTAMP`,
        [userId, newBonusBalance]
      );

      // Create bonus transaction
      await client.query(
        `INSERT INTO transactions (user_id, type, amount, balance_before, balance_after, currency, status, description, reference_id, created_at)
         VALUES ($1, 'bonus', $2, $3, $4, 'USD', 'completed', $5, $6, CURRENT_TIMESTAMP)`,
        [userId, bonusAmount, currentBonusBalance, newBonusBalance, `Bonus from ${promotion.name}`, `promo_${promotion_id}`]
      );
    }
...
export const getDailySpin = async (req: Request, res: Response): Promise<void> => {
  try {
    const userId = (req as any).user?.id;
    
    // Check if user already spun today
    const today = new Date().toISOString().split('T')[0];
    const existingSpinResult = await pool.query(
      "SELECT * FROM user_activity_logs WHERE user_id = $1 AND action = 'daily_spin' AND DATE(created_at) = $2",
      [userId, today]
    );
...
// Perform daily spin
export const performDailySpin = async (req: Request, res: Response): Promise<void> => {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    ...
    // Check if user already spun today
    const today = new Date().toISOString().split('T')[0];
    const existingSpinResult = await client.query(
      "SELECT * FROM user_activity_logs WHERE user_id = $1 AND action = 'daily_spin' AND DATE(created_at) = $2",
      [userId, today]
    );
    ...
    // Log the spin
    await client.query(
      `INSERT INTO user_activity_logs (user_id, action, category, description, metadata, created_at)
       VALUES ($1, 'daily_spin', 'bonus', $2, $3, CURRENT_TIMESTAMP)`,
      [userId, `Daily spin result: ${spinResult.type}`, JSON.stringify(spinResult)]
    );
    ...
    if (spinResult.type === 'bonus' && spinResult.amount > 0) {
      ...
      await client.query(
        `INSERT INTO transactions (user_id, type, amount, balance_before, balance_after, currency, status, description, reference_id, created_at)
         VALUES ($1, 'bonus', $2, $3, $4, 'USD', 'completed', $5, $6, CURRENT_TIMESTAMP)`,
        [userId, spinResult.amount, currentBonusBalance, newBonusBalance, `Daily spin bonus: ${spinResult.description}`, `daily_spin_${Date.now()}`]
      );
    }
...
export const getWageringProgress = async (req: Request, res: Response): Promise<void> => {
  try {
    const userId = (req as any).user?.id;
    
    const wageringProgress = await PromotionService.getWageringProgress(userId);
...
export const getBonusBalanceSummary = async (req: Request, res: Response): Promise<void> => {
  try {
    const userId = (req as any).user?.id;
    
    const bonusSummary = await PromotionService.getBonusBalanceSummary(userId);
...
export const transferBonusToMain = async (req: Request, res: Response): Promise<void> => {
  try {
    const userId = (req as any).user?.id;
...
export const checkPromotionEligibility = async (req: Request, res: Response): Promise<void> => {
  try {
    const userId = (req as any).user?.id;
...
// Helper functions
function isPromotionEligible(promotion: any, userId: number): boolean {
  // Basic eligibility check - can be expanded based on business rules
  return true;
}

async function checkPromotionEligibilityHelper(client: any, userId: number, promotion: any): Promise<{ eligible: boolean; reason?: string }> {
  if (promotion.type === 'welcome_bonus') {
    // Check if user has claimed any welcome bonus before
    const existingWelcomeBonus = await client.query(
      `SELECT up.id FROM user_promotions up 
       JOIN promotions p ON up.promotion_id = p.id 
       WHERE up.user_id = $1 AND p.type = 'welcome_bonus'`,
      [userId]
    );
...
function generateSpinResult(): any {
  const spinTypes = [
  ...
  try {
    ...
    if (userId) {
      ...
      if (promotion.max_claims_per_user) {
        const userClaimsResult = await pool.query(
          `SELECT COUNT(*) as claim_count FROM user_promotions WHERE user_id = $1 AND promotion_id = $2`,
          [userId, promotion.id]
        );
...
export const applyPromoCode = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const userPromotionResult = await client.query(
      `INSERT INTO user_promotions (user_id, promotion_id, status, bonus_amount, claimed_at, created_at, updated_at)
       VALUES ($1, $2, 'active', $3, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
       RETURNING *`,
      [userId, promotion.id, bonusAmount]
    );
...
export const autoApplyBestPromotion = async (req: Request, res: Response): Promise<void> => {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    ...
    const promotionResult = await client.query(
      `SELECT p.*
       ...
       LIMIT 1`,
      [userId, promotionType, deposit_amount]
    );
