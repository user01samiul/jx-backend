...
export const createDeposit = async (
  ...
  try {
    ...
    const conversionResult = await paymentService.convertCryptoToUSD(
      gateway_code,
      {
...
export const checkPaymentStatus = async (
  ...
  try {
    ...
    const transactionResult = await pool.query(
      `SELECT t.*, pg.code as gateway_code, pg.api_key, pg.api_secret, pg.api_endpoint, pg.webhook_url, pg.webhook_secret, pg.config
       FROM transactions t
    ...
    if (statusResponse.success && statusResponse.status !== transaction.status) {
      await pool.query(
        ...
        [statusResponse.status, transaction_id]
      );
      ...
      if (statusResponse.status === 'completed' && transaction.status !== 'completed') {
        try {
          ...
          const balanceResult = await BalanceService.processDeposit(
            userId,
            usdAmount,  // USD amount for balance
            `Deposit of ${cryptoAmount} ${cryptoCurrency} ($${usdAmount.toFixed(2)} USD) completed`,
...
export const handleWebhook = async (
  ...
  try {
    ...
    let transactionResult = await pool.query(
      `SELECT * FROM transactions WHERE reference_id = $1 OR external_reference = $1`,
    ...
    if (transactionResult.rows.length === 0) {
      ...
      transactionResult = await pool.query(
        `SELECT * FROM transactions WHERE metadata->>'gateway_response'->>'id' = $1 OR metadata->>'gateway_response'->>'invoice_id' = $1`,
        [webhookResult.transaction_id]
      );
    }
...
// Rate cache for crypto-to-USD conversion (60 second TTL)
 ...
 */
export const getCryptoConversion = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    ...
    const cryptoCurrency = (currency as string).toUpperCase();
    const cryptoAmount = parseFloat(amount as string);
