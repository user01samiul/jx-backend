...
export const getAllSupportUsers = async (req: Request, res: Response) => {
  try {
    const result = await pool.query(
      `SELECT u.id, u.username, u.email, u.created_at, u.status_id,
...
export const getSupportUserById = async (req: Request, res: Response) => {
  try {
    ...
    const result = await pool.query(
      `SELECT u.id, u.username, u.email, u.created_at, u.status_id,
       ...
       LEFT JOIN statuses s ON u.status_id = s.id
       LEFT JOIN chat_agents ca ON u.id = ca.user_id
       WHERE u.id = $1 AND ur.role_id IN (1, 3)`,
      [userId]
    );
...
export const createSupportUser = async (req: Request, res: Response) => {
  try {
    const {
      ...
      email,
      password,
      role_id = 3, // Default to Support role
    ...
    try {
      ...
      const userResult = await client.query(
        `INSERT INTO users (username, email, password, status_id, created_by)
         VALUES ($1, $2, $3, 1, $4)
         RETURNING id, username, email, created_at`,
        [username, email, hashedPassword, (req as any).user.userId]
      );
      ...
      await client.query(
        `INSERT INTO user_roles (user_id, role_id, created_by)
         VALUES ($1, $2, $3)`,
        [newUser.id, role_id, (req as any).user.userId]
      );

      // Create agent profile if Support role
      if (role_id === 3 || role_id === 1) {
        await client.query(
          `INSERT INTO chat_agents (user_id, status, max_concurrent_chats, specializations, languages)
           VALUES ($1, 'offline', $2, $3, $4)`,
          [newUser.id, max_concurrent_chats, specializations, languages]
        );
      }
...
// Update support user
export const updateSupportUser = async (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
    const {
      email,
    ...
    try {
      ...
      if (userUpdates.length > 0) {
        ...
        userValues.push((req as any).user.userId);
      ...
      if (role_id && [1, 3].includes(role_id)) {
        await client.query(
          `UPDATE user_roles SET role_id = $1, updated_by = $2, updated_at = CURRENT_TIMESTAMP
           WHERE user_id = $3`,
          [role_id, (req as any).user.userId, userId]
        );
      }
      ...
      if (agentUpdates.length > 0) {
        ...
        await client.query(
          `UPDATE chat_agents SET ${agentUpdates.join(', ')} WHERE user_id = $${paramCount}`,
          agentValues
        );
      }
...
export const deleteSupportUser = async (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
    ...
    if (parseInt(userId) === (req as any).user.userId) {
      return res.status(400).json({
        success: false,
    ...
    await pool.query(
      `UPDATE users SET status_id = 2, updated_by = $1, updated_at = CURRENT_TIMESTAMP
       WHERE id = $2`,
      [(req as any).user.userId, userId]
    );

    // Set agent status to offline
    await pool.query(
      `UPDATE chat_agents SET status = 'offline', updated_at = CURRENT_TIMESTAMP
       WHERE user_id = $1`,
      [userId]
    );
...
export const getSupportUserStats = async (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
    ...
    const result = await pool.query(
      `SELECT
       ...
       GROUP BY ca.user_id, ca.total_chats_handled, ca.avg_rating,
                ca.avg_response_time, ca.current_chat_count, ca.max_concurrent_chats`,
      [userId]
    );
