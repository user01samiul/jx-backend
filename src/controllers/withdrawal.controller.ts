...
export class WithdrawalController {
      ...
      if (payment_method === 'crypto') {
        ...
        if (currency && currency !== 'crypto') {
          ...
          const gatewayResult = await pool.query(
            'SELECT supported_currencies FROM payment_gateways WHERE code = $1 AND is_active = true',
            ['oxapay'] // Using oxapay as default gateway for crypto
          );
  ...
  static async getPaymentMethodStats(req: Request, res: Response) {
    try {
      const stats = await WithdrawalService.getPaymentMethodStats();
