 ...
 */
export async function getPlayer360View(req: Request, res: Response) {
  ...
  try {
    ...
    try {
      ...
      const [
        ...
        // Recent transactions
        client.query(
          `SELECT id, type, amount, currency, status, payment_method, created_at, created_at as completed_at
           FROM transactions
           WHERE user_id = $1
      ...
      const playerValueScore = Math.min(
        100,
        Math.floor(
          (ltv / 1000) * 30 + // 30 points for every $1000 LTV
...
function getTimelineColor(category: string): string {
  switch (category) {
    case "financial":
      return "bg-green-100";
    case "gaming":
  ...
  try {
    const result = await pool.query(
      `INSERT INTO customer_notes
       ...
       RETURNING *`,
      [userId, agentId, note_type, category, subject, content, sentiment, is_important, is_flagged, tags || [], agentId]
    );
    ...
    await pool.query(
      `INSERT INTO customer_timeline
       ...
       VALUES ($1, 'note_added', 'support', $2, $3)`,
      [userId, JSON.stringify({ note_id: result.rows[0].id, subject }), agentId]
    );
 ...
 */
export async function addPlayerTag(req: Request, res: Response) {
  const { userId } = req.params;
  ...
  try {
    const result = await pool.query(
      `INSERT INTO user_tags (user_id, tag_id, tagged_by, auto_tagged)
       VALUES ($1, $2, $3, false)
       ON CONFLICT (user_id, tag_id) DO NOTHING
       RETURNING *`,
      [userId, tag_id, agentId]
    );
 ...
 */
export async function getPlayerGameHistory(req: Request, res: Response) {
  const { userId } = req.params;
  ...
  try {
    ...
    try {
      ...
      const countQuery = await client.query(
        `SELECT COUNT(*) as total FROM bets WHERE user_id = $1`,
        [userId]
      );
 ...
 */
export async function getHandHistory(req: Request, res: Response) {
  ...
  try {
    ...
    const handHistory = await InnovaApiService.getHandHistory(
      bet.transaction_id,
      historyId,
      bet.game_vendor
    );
