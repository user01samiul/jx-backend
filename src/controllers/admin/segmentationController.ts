...
class SegmentationController {
  ...
  async getAvailableFilters(req: Request, res: Response) {
    try {
      ...
      const filters = await segmentationService.getAvailableFilters();
  ...
  async previewSegment(req: Request, res: Response) {
    try {
      ...
      const playerIds = await segmentationService.applySegment(filters, 10000);
      const samplePlayers = await segmentationService.getSegmentPlayers(filters, 1, Math.min(limit, 100));

      console.log(`[SEGMENTATION] Segment matched ${playerIds.length} players`);

      const categorySet = new Set(filters.map((f: any) => f.category));
   ...
   */
  async querySegment(req: Request, res: Response) {
    try {
      ...
      const result = await segmentationService.getSegmentPlayers(filters, page, limit);
   ...
   */
  async saveSegment(req: Request, res: Response) {
    try {
      ...
      const playerIds = await segmentationService.applySegment(filters, 100000);

      // Save segment
      const segmentId = await segmentationService.saveSegment({
        name,
        description,
        filters,
      ...
      // Update player count
      await segmentationService.updateSegmentPlayerCount(segmentId, playerIds);
   ...
   */
  async getSavedSegments(req: Request, res: Response) {
    try {
      console.log('[SEGMENTATION] Fetching saved segments...');

      const segments = await segmentationService.getSavedSegments();
   ...
   */
  async exportSegment(req: Request, res: Response) {
    try {
      ...
      const playerIds = await segmentationService.applySegment(filters, 100000);
      const players = await segmentationService.getSegmentPlayers(filters, 1, 10000);

      if (format === 'csv') {
        // CSV export
        const csvHeader = 'ID,Username,Email,Country,VIP Tier,Current Balance,Total Deposited,Total Wagered\n';
        const csvRows = players.players.map((p: any) =>
          `${p.id},"${p.username}","${p.email}","${p.country || ''}","${p.vip_tier || ''}",${p.current_balance || 0},${p.total_deposited || 0},${p.total_wagered || 0}`
        ).join('\n');
   ...
   */
  async analyzeSegment(req: Request, res: Response) {
    try {
  ...
  private aggregateField(players: any[], field: string): { [key: string]: number } {
...
export const getAvailableFilters = segmentationController.getAvailableFilters.bind(segmentationController);
export const previewSegment = segmentationController.previewSegment.bind(segmentationController);
export const querySegment = segmentationController.querySegment.bind(segmentationController);
export const saveSegment = segmentationController.saveSegment.bind(segmentationController);
export const getSavedSegments = segmentationController.getSavedSegments.bind(segmentationController);
export const exportSegment = segmentationController.exportSegment.bind(segmentationController);
export const analyzeSegment = segmentationController.analyzeSegment.bind(segmentationController);
