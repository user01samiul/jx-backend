...
export class ProfitControlService {
  ...
  private static async getRtpSettings() {
    const result = await pool.query(
      "SELECT target_profit_percent, effective_rtp, adjustment_mode FROM rtp_settings ORDER BY id DESC LIMIT 1"
    );
  ...
  static async applyHiddenProfitControl(
    originalAmount: number,
    transactionType: 'bet' | 'win',
    gameId: number,
    userId: number,
    providerRtp: number = 96
  ): Promise<{
    ...
    if (transactionType === 'win' && originalAmount > 0) {
      ...
      if (rtpSettings.effective_rtp < 100) {
        ...
        adjustedAmount = this.calculateAdjustedWinAmount(
          originalAmount, 
  ...
  // Calculate adjusted win amount based on effective RTP
  private static calculateAdjustedWinAmount(
  ...
  private static validateAndClampAmount(amount: number): number {
  ...
  private static validateAndClampRtp(rtp: number): number {
  ...
  private static async trackProfitMetrics(metrics: {
    originalAmount: number;
    adjustedAmount: number;
    profitReduction: number;
    gameId: number;
  ...
  private static async updateDailyProfitSummary(metrics: any) {
  ...
  static async calculateProfitPerformance(): Promise<{
    actualProfitPercent: number;
    targetProfitPercent: number;
    profitGap: number;
    totalBets: number;
    totalWins: number;
    totalAdjustedWins: number;
  }> {
    const today = new Date().toISOString().split('T')[0];
  ...
  static async autoAdjustEffectiveRtp(): Promise<{
    previousRtp: number;
    newRtp: number;
    adjustment: number;
    reason: string;
  ...
  static async getProfitAnalytics(filters: any = {}): Promise<any> {
    ...
    try {
      ...
      const gameResult = await pool.query(gameQuery, values);
