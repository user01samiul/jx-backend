 ...
 */
export class MongoHybridService {
  constructor() {
  ...
  private async ensureMongoInitialized() {
    try {
      // Try to get a collection to see if MongoDB is initialized
      MongoService.getBetsCollection();
    } catch (error) {
  ...
  async insertTransaction(transactionData: any): Promise<any> {
    await this.ensureMongoInitialized();
    return await MongoService.insertTransaction(transactionData);
  }

  async getTransaction(id: number): Promise<any> {
    await this.ensureMongoInitialized();
    return await MongoService.getTransactionById(id);
  }

  async getTransactions(userId?: number, limit: number = 50): Promise<any[]> {
    await this.ensureMongoInitialized();
    return await MongoService.getTransactions(userId, limit);
  }

  async updateTransaction(id: number, updateData: any): Promise<any> {
    await this.ensureMongoInitialized();
    return await MongoService.updateTransaction(id, updateData);
  }
  ...
  async insertBet(betData: any): Promise<any> {
    await this.ensureMongoInitialized();
    return await MongoService.insertBet(betData);
  }

  async getBet(id: number): Promise<any> {
    await this.ensureMongoInitialized();
    return await MongoService.getBetById(id);
  }

  async getBetById(betId: number): Promise<any> {
    await this.ensureMongoInitialized();
    return await MongoService.getBetById(betId);
  }

  async getBets(userId?: number, limit: number = 50): Promise<any[]> {
    await this.ensureMongoInitialized();
    return await MongoService.getBets(userId, limit);
  }

  async updateBet(id: number, updateData: any): Promise<any> {
    await this.ensureMongoInitialized();
    return await MongoService.updateBet(id, updateData);
  }
  ...
  async getCategoryBalance(userId: number, category: string): Promise<number> {
    await this.ensureMongoInitialized();
    const balance = await MongoService.getCategoryBalance(userId, category);
    return balance || 0;
  }

  async updateCategoryBalance(userId: number, category: string, amount: number): Promise<any> {
    await this.ensureMongoInitialized();

    return await MongoService.updateCategoryBalance(userId, category, amount);
  }

  async addCategoryBalance(userId: number, category: string, amount: number): Promise<any> {
    await this.ensureMongoInitialized();
    return await MongoService.addCategoryBalance(userId, category, amount);
  }

  async deductCategoryBalance(userId: number, category: string, amount: number): Promise<any> {
    await this.ensureMongoInitialized();
    return await MongoService.deductCategoryBalance(userId, category, amount);
  }

  async getUserCategoryBalances(userId: number): Promise<any[]> {
    await this.ensureMongoInitialized();
    return await MongoService.getUserCategoryBalances(userId);
  }
   ...
   */
  async placeBet(userId: number, gameId: number, betAmount: number, gameData?: any): Promise<any> {
   ...
   */
  async processBetResult(betId: number, outcome: 'win' | 'lose', winAmount: number = 0, gameResult?: any): Promise<any> {
    const client = await pool.connect();
    ...
    try {
      ...
      // Get game data from PostgreSQL
      const gameResult = await client.query(
        "SELECT name, category FROM games WHERE id = $1",
        [bet.game_id]
      );
  ...
  async getUserBalance(userId: number): Promise<any> {
    ...
    try {
      ...
      const mainBalanceResult = await client.query(
        'SELECT balance, total_deposited, total_withdrawn FROM user_balances WHERE user_id = $1',
        [userId]
      );
   ...
   */
  async adminTopup(userId: number, amount: number, description: string = 'Admin top-up'): Promise<any> {
    const client = await pool.connect();
    ...
    try {
      ...
      const currentBalanceResult = await client.query(
        'SELECT balance, total_deposited FROM user_balances WHERE user_id = $1',
        [userId]
      );
      ...
      const transactionResult = await client.query(
        `INSERT INTO transactions (user_id, type, amount, balance_before, balance_after, currency, status, description, metadata, created_at) 
      ...
      await client.query(
        `INSERT INTO user_balances (user_id, balance, total_deposited, updated_at) 
         VALUES ($1, $2, $3, NOW())
         ON CONFLICT (user_id) 
         DO UPDATE SET 
           balance = $2, 
           total_deposited = $3, 
           updated_at = NOW()`,
        [userId, newBalance, newTotalDeposited]
      );
   ...
   */
  static getMongoService(): typeof MongoService {
    return MongoService;
  }
