...
export class AffiliateApplicationService {
   ...
   */
  static async submitApplication(data: CreateApplicationData): Promise<AffiliateApplication> {
    ...
    try {
      ...
      const existingCheck = await client.query(
        `SELECT id FROM affiliate_applications
         WHERE user_id = $1 AND application_status = 'pending'`,
        [data.userId]
      );

      if (existingCheck.rows.length > 0) {
        throw new ApiError('You already have a pending application', 400);
      }
      ...
      if (affiliateCheck.rows.length > 0) {
        throw new ApiError('You are already an affiliate', 400);
      }
      ...
      if (data.uplineReferralCode) {
        ...
        if (uplineResult.rows.length === 0) {
          throw new ApiError('Invalid upline referral code', 400);
        }
   ...
   */
  static async getUserApplicationStatus(userId: number): Promise<any> {
    const result = await pool.query(
  ...
  static async approveApplication(
    ...
    approvedBy: number,
    approvalData: ApproveApplicationData
  ): Promise<any> {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
      ...
      const appResult = await client.query(
        `SELECT * FROM affiliate_applications WHERE id = $1 AND application_status = 'pending'`,
        [applicationId]
      );

      if (appResult.rows.length === 0) {
        throw new ApiError('Application not found or already processed', 404);
      }
      ...
      if (existingProfile.rows.length > 0) {
        throw new ApiError('User already has an affiliate profile', 400);
      }
      ...
      const codeCheck = await client.query(
        'SELECT id FROM affiliate_profiles WHERE referral_code = $1',
        [referralCode]
      );

      if (codeCheck.rows.length > 0) {
        throw new ApiError('Referral code already exists. Please try a different one.', 400);
      }
      ...
      if (application.upline_referral_code) {
        const uplineResult = await client.query(
          ...
          [application.upline_referral_code]
        );
      ...
      if (roleResult.rows.length > 0) {
        await client.query(
          ...
          [application.user_id, roleResult.rows[0].id]
        );
      }
      ...
      await client.query(
        `UPDATE affiliate_applications
         SET application_status = 'approved',
             ...
             admin_notes = $2
         WHERE id = $3`,
        [approvedBy, approvalData.adminNotes, applicationId]
      );
  ...
  static async rejectApplication(
    applicationId: number,
    rejectedBy: number,
    rejectData: RejectApplicationData
  ): Promise<any> {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
      ...
      const result = await client.query(
        `UPDATE affiliate_applications
         ...
         WHERE id = $4
         RETURNING *`,
        [rejectedBy, rejectData.rejectionReason, rejectData.adminNotes, applicationId]
      );
   ...
   * Generate unique referral code
   */
  private static async generateReferralCode(client: any): Promise<string> {
    for (let i = 0; i < 10; i++) {
   ...
   */
  static async getApplicationStatistics(): Promise<any> {
    const result = await pool.query(`
      SELECT
