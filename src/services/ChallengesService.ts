...
interface ChallengeTemplate {
  ...
  name: string;
  description: string;
  type: 'DEPOSIT' | 'WAGER' | 'GAME_PLAY' | 'WIN_COUNT' | 'STREAK' | 'TOURNAMENT' | 'REFERRAL';
  target_value: number;
  reward_type: 'CASH' | 'BONUS' | 'FREE_SPINS' | 'LOYALTY_POINTS';
...
class ChallengesService {
   ...
   */
  async createTemplate(template: ChallengeTemplate): Promise<any> {
   ...
   */
  async getAllTemplates(status?: string): Promise<any[]> {
   ...
   */
  async updateTemplate(templateId: number, updates: Partial<ChallengeTemplate>): Promise<any> {
    const fields: string[] = [];
    const values: any[] = [];
    let paramIndex = 1;
    ...
    const result = await pool.query(
      `UPDATE challenge_templates SET ${fields.join(', ')}, updated_at = CURRENT_TIMESTAMP
   ...
   */
  async assignChallengeToPlayer(userId: number, templateId: number): Promise<any> {
    ...
    const existingChallenge = await pool.query(
      `SELECT id FROM player_challenges
    ...
    const result = await pool.query(
      ...
      RETURNING *`,
      [userId, templateId, template.target_value, expiresAt]
    );
   ...
   */
  async getPlayerChallenges(userId: number, status?: string): Promise<any[]> {
  ...
  async updateChallengeProgress(
    ...
    try {
      ...
      if (challenge.expires_at && new Date(challenge.expires_at) < new Date()) {
        await client.query(
          `UPDATE player_challenges SET status = 'EXPIRED' WHERE id = $1`,
          [challenge.id]
        );
   ...
   */
  async claimChallengeReward(userId: number, challengeId: number): Promise<any> {
    const client = await pool.connect();
    ...
    try {
      ...
      const challengeResult = await client.query(
        `SELECT pc.*, ct.reward_type, ct.reward_value
         FROM player_challenges pc
         JOIN challenge_templates ct ON pc.template_id = ct.id
      ...
      // Mark as claimed
      await client.query(
        `UPDATE player_challenges SET status = 'CLAIMED', claimed_at = CURRENT_TIMESTAMP
         WHERE id = $1`,
        [challengeId]
      );
      ...
      switch (challenge.reward_type) {
        ...
        case 'LOYALTY_POINTS':
          await client.query(
            `UPDATE player_loyalty SET available_points = available_points + $1, lifetime_points = lifetime_points + $1
             WHERE user_id = $2`,
            [challenge.reward_value, userId]
          );
          break;
   ...
   */
  async autoAssignChallenges(): Promise<void> {
  ...
  private async grantFreeSpinsCampaign(
    ...
    try {
      ...
      const campaignCode = InnovaCampaignsService.generateCampaignCode('challenge', challengeId, userId);
