...
interface TransactionData {
  user_id: number;
  type: 'bet' | 'win' | 'deposit' | 'withdrawal' | 'bonus' | 'cashback' | 'refund' | 'adjustment';
  amount: number;
  currency: string;
  description: string;
...
export class BalanceConsistencyService {
  /**
   * Get comprehensive balance information for a user
   */
  static async getUserBalanceInfo(userId: number, excludeTransactionId?: string): Promise<BalanceInfo> {
    ...
    try {
      ...
      const mainBalance = CurrencyUtils.safeParseBalance(mainBalanceResult.rows[0]?.main_balance || 0);
      const totalDeposited = CurrencyUtils.safeParseBalance(mainBalanceResult.rows[0]?.total_deposited || 0);
      const totalWithdrawn = CurrencyUtils.safeParseBalance(mainBalanceResult.rows[0]?.total_withdrawn || 0);
      const totalWagered = CurrencyUtils.safeParseBalance(mainBalanceResult.rows[0]?.total_wagered || 0);
      const totalWon = CurrencyUtils.safeParseBalance(mainBalanceResult.rows[0]?.total_won || 0);
      ...
      const totalDeposits = CurrencyUtils.safeParseBalance(depositsResult.rows[0]?.total_deposits || 0);
      const totalWithdrawals = CurrencyUtils.safeParseBalance(depositsResult.rows[0]?.total_withdrawals || 0);
      ...
      categoryBalancesResult.rows.forEach(row => {
        if (row.category) {
          ...
          categoryBalances[row.category] = CurrencyUtils.safeParseBalance(row.category_balance);
        }
      });
      ...
      const lockedAmount = CurrencyUtils.safeParseBalance(lockedAmountResult.rows[0]?.locked_amount || 0);
   ...
   */
  static async validateBetBalance(
    userId: number, 
    betAmount: number, 
    category?: string
   ...
   */
  static async processTransactionWithBalanceUpdate(
    transactionData: TransactionData,
    category?: string
  ): Promise<{ transaction_id: number; balance_before: number; balance_after: number }> {
    const client = await pool.connect();
    ...
    try {
      ...
      if (category) {
        ...
        const categoryNetBalance = CurrencyUtils.safeParseBalance(categoryBalanceResult.rows[0]?.category_balance || 0);
        const mainBalance = CurrencyUtils.safeParseBalance(mainBalanceResult.rows[0]?.main_balance || 0);
        ...
        if (transactionData.type === 'bet') {
          ...
          if (totalAvailable < transactionData.amount) {
            throw new ApiError(`Insufficient balance. Category available: ${balanceBefore}, Main available: ${mainBalance}, Required: ${transactionData.amount}`, 400);
          }
          // Use proper decimal arithmetic to avoid floating-point precision issues
          balanceAfter = CurrencyUtils.subtract(balanceBefore, transactionData.amount);
          console.log(`[BALANCE_CONSISTENCY] BET: ${balanceBefore} - ${transactionData.amount} = ${balanceAfter}`);
        } else {
          // Use proper decimal arithmetic to avoid floating-point precision issues
          balanceAfter = CurrencyUtils.add(balanceBefore, transactionData.amount);
        ...
        balanceBefore = CurrencyUtils.safeParseBalance(balanceResult.rows[0]?.main_balance || 0);
        ...
        if (transactionData.type === 'bet' || transactionData.type === 'withdrawal') {
          if (balanceBefore < transactionData.amount) {
            throw new ApiError(`Insufficient main balance. Available: ${balanceBefore}, Required: ${transactionData.amount}`, 400);
          }
          // Use proper decimal arithmetic to avoid floating-point precision issues
          balanceAfter = CurrencyUtils.subtract(balanceBefore, transactionData.amount);
   ...
   */
  static async syncUserBalance(userId: number): Promise<void> {
    const client = await pool.connect();
   ...
   */
  static async fixBalanceInconsistencies(userId: number): Promise<{
    fixed: boolean;
    oldMainBalance: number;
    newMainBalance: number;
    oldCategoryBalances: Record<string, number>;
    newCategoryBalances: Record<string, number>;
  }> {
    const client = await pool.connect();
    ...
    try {
      ...
      const oldCategoryBalances: Record<string, number> = {};
   ...
   */
  static async syncStoredBalances(userId: number): Promise<void> {
    const client = await pool.connect();
   ...
   */
  static async getBalanceAuditTrail(userId: number, limit: number = 50): Promise<any[]> {
