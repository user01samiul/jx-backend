...
export class MongoBalanceSyncService {
  /**
   ...
   */
  static async syncCategoryBalance(userId: number, category: string): Promise<number> {
    ...
    if (hasManualTransfers) {
      ...
      const currentStoredBalance = await MongoService.getCategoryBalance(userId, category);
    ...
    await MongoService.updateCategoryBalance(userId, category, categoryBalance);
   ...
   */
  static async syncUserBalance(userId: number): Promise<BalanceInfo> {
    ...
    try {
      ...
      const bonusBalanceResult = await client.query(
        'SELECT COALESCE(bonus_balance, 0) as bonus_balance FROM user_balances WHERE user_id = $1',
        [userId]
      );
      ...
      const currencyResult = await client.query(
        'SELECT COALESCE(currency, \'USD\') as currency FROM user_profiles WHERE user_id = $1',
        [userId]
      );
   ...
   */
  private static async hasManualTransfers(userId: number, category: string): Promise<boolean> {
    const transactions = await MongoService.getTransactionsCollection().find({
      user_id: userId,
      status: 'completed',
   ...
   */
  private static async calculateNonTransferBalance(userId: number, category: string): Promise<number> {
    const transactions = await MongoService.getTransactionsCollection().find({
      user_id: userId,
      status: 'completed',
      'metadata.category': category.toLowerCase().trim(),
   ...
   */
  private static async getMainWalletTotals(userId: number): Promise<{ totalDeposits: number; totalWithdrawals: number }> {
    const client = await pool.connect();
   ...
   */
  private static async calculateRealTimeBalance(userId: number): Promise<number> {
    const client = await pool.connect();
   ...
   */
  static async syncAllCategoryBalances(userId: number): Promise<Record<string, number>> {
    console.log(`[MONGO_SYNC] Syncing all category balances for user ${userId}`);
    
    // Get all unique categories from MongoDB transactions
    const categories = await MongoService.getTransactionsCollection().distinct('metadata.category', {
      user_id: userId,
   ...
   */
  static async syncUserFullBalance(userId: number): Promise<{
