...
import { BalanceMongoService } from "./balance-mongo.service";
...
export const getUserWithBalanceService = async (userId: number) => {
  ...
  const balanceResult = await pool.query(
    `SELECT balance, bonus_balance, locked_balance, total_deposited, total_withdrawn, total_wagered, total_won FROM user_balances WHERE user_id = $1 LIMIT 1`,
    [userId]
  );
  ...
  const mainBalance = CurrencyUtils.safeParseBalance(balanceRow.balance);
  const categorySum = categoryBalances.reduce((sum, c) => {
    const categoryBalance = CurrencyUtils.safeParseBalance(c.balance);
  ...
  return {
    ...
    categories: {
      category_balances: categoryBalances.map(c => ({
        category: c.category,
        balance: CurrencyUtils.safeParseBalance(c.balance).toString()
      }))
    }
  };
};
...
export const getUserRecentActivityService = async (userId: number, limit: number = 20) => {
  ...
  const countResult = await pool.query(
    `SELECT COUNT(*) as total_count FROM user_activity_logs WHERE user_id = $1`,
    [userId]
  );
...
export const updateUserProfileService = async (userId: number, profileData: any) => {
...
export const changePasswordService = async (userId: number, passwordData: any) => {
  const client = await pool.connect();
  ...
  try {
    ...
    // Update password
    await client.query(
      'UPDATE users SET password = $1, updated_at = CURRENT_TIMESTAMP, updated_by = $2 WHERE id = $3',
      [hashedNewPassword, userId, userId]
    );

    // Log password change activity
    await client.query(
      'INSERT INTO user_activity_logs (user_id, action, category, description, created_by) VALUES ($1, $2, $3, $4, $1)',
      [userId, 'change_password', 'security', 'Password changed successfully']
    );
...
export const enable2FAService = async (userId: number) => {
  const client = await pool.connect();
  ...
  try {
    ...
    const userResult = await client.query(
      'SELECT auth_secret, qr_code, is_2fa_enabled FROM users WHERE id = $1',
      [userId]
    );
    ...
    await client.query(
      'UPDATE users SET is_2fa_enabled = TRUE, updated_at = CURRENT_TIMESTAMP, updated_by = $1 WHERE id = $2',
      [userId, userId]
    );
    ...
    await client.query(
      'INSERT INTO user_activity_logs (user_id, action, category, description, created_by) VALUES ($1, $2, $3, $4, $1)',
      [userId, 'enable_2fa', 'security', '2FA enabled successfully']
    );
...
export const disable2FAService = async (userId: number) => {
  const client = await pool.connect();
  ...
  try {
    ...
    await client.query(
      'UPDATE users SET is_2fa_enabled = FALSE, updated_at = CURRENT_TIMESTAMP, updated_by = $1 WHERE id = $2',
      [userId, userId]
    );
    ...
    await client.query(
      'INSERT INTO user_activity_logs (user_id, action, category, description, created_by) VALUES ($1, $2, $3, $4, $1)',
      [userId, 'disable_2fa', 'security', '2FA disabled successfully']
    );
...
export const skip2FAService = async (userId: number, authData: any) => {
  const client = await pool.connect();
  ...
  try {
    ...
    const userResult = await client.query(
      'SELECT password, auth_secret, is_2fa_enabled FROM users WHERE id = $1',
      [userId]
    );
    ...
    await client.query(
      'UPDATE users SET is_2fa_enabled = FALSE, auth_secret = NULL, qr_code = NULL, updated_at = CURRENT_TIMESTAMP, updated_by = $1 WHERE id = $2',
      [userId, userId]
    );
    ...
    await client.query(
      'INSERT INTO user_activity_logs (user_id, action, category, description, created_by) VALUES ($1, $2, $3, $4, $1)',
      [userId, 'skip_2fa', 'security', '2FA setup skipped by user']
    );
 ...
 */
export const getUserCategoryBalancesService = async (userId: number) => {
  ...
  const categoriesResult = await pool.query(
    `SELECT DISTINCT category FROM games WHERE is_active = TRUE`
  );
 ...
 */
export const transferUserCategoryBalanceService = async (
  userId: number,
  category: string,
  amount: number,
  direction: 'main_to_category' | 'category_to_main'
) => {
  ...
  try {
    ...
    if (mainResult.rows.length === 0) {
      ...
      mainBalance = CurrencyUtils.safeParseBalance(mainResult.rows[0].balance);
    }
    ...
    if (direction === 'main_to_category') {
      if (mainBalance < amount) throw new Error('Insufficient main balance');
      mainBalance -= amount;
      catBalance += amount;
    } else if (direction === 'category_to_main') {
      if (catBalance < amount) throw new Error('Insufficient category balance');
      catBalance -= amount;
      mainBalance += amount;
    } else {
 ...
 */
export const getUserGameBetsService = async (userId: number) => {
  const result = await pool.query(
    `SELECT
      ugb.game_id,
      g.name as game_name,
    ...
    WHERE ugb.user_id = $1
    ORDER BY ugb.last_bet_at DESC NULLS LAST, ugb.last_result_at DESC NULLS LAST, g.name ASC`,
    [userId]
  );
  return result.rows;
};