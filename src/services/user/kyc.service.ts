 ...
 */
export const getKYCStatusService = async (userId: number) => {
  ...
  try {
    ...
    const documentCounts = documentsCountResult.rows.reduce((acc: any, row: any) => {
    ...
    const approvedDocumentTypes = approvedDocsResult.rows.map((row: any) => row.document_type);
    ...
    const pendingDocumentTypes = pendingDocsResult.rows.map((row: any) => row.document_type);
    ...
    const rejectedDocumentTypes = rejectedDocsResult.rows.map((row: any) => row.document_type);
 ...
 */
export const getKYCDocumentsService = async (userId: number) => {
  const query = `
    SELECT
      id,
      document_type,
 ...
 */
export const uploadKYCDocumentService = async (
  userId: number,
  documentData: {
 ...
 */
export const deleteKYCDocumentService = async (userId: number, documentId: number) => {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
 ...
 */
export const getKYCRequirementsService = (level: number) => {
  const requirements = [
    {
      level: 0,
      name: 'Unverified',
 ...
 */
const getRequiredDocumentsForLevel = (level: number): string[] => {
  const requirements = getKYCRequirementsService(level);
  return requirements.required_documents.map((doc: any) => doc.type);
};
