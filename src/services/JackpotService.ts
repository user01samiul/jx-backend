...
interface JackpotSchedule {
  id?: number;
  name: string;
  type: 'CASINO' | 'HAPPYHOUR' | 'TEST' | 'GOOD_WIN';
  vendor?: string;
  wallet_group?: string;
...
interface JackpotInstance {
  id?: number;
  schedule_id: number;
  current_size: number;
  seed_size: number;
  winner_user_id?: number;
  winner_wallet_name?: string;
  won_at?: Date;
...
class JackpotService {
   ...
   */
  async createJackpotSchedule(schedule: JackpotSchedule): Promise<number> {
    const result = await pool.query(
      `INSERT INTO jackpot_schedules (name, type, vendor, wallet_group, currency_code, seed_amount, contribution_percentage, status)
   ...
   */
  async startJackpotInstance(scheduleId: number): Promise<number> {
    // Get schedule details
    const scheduleResult = await pool.query(
      'SELECT * FROM jackpot_schedules WHERE id = $1',
      [scheduleId]
    );
    ...
    // Create new instance
    const result = await pool.query(
      `INSERT INTO jackpot_instances (schedule_id, current_size, seed_size, status)
       VALUES ($1, $2, $3, 'ACTIVE')
       RETURNING id`,
      [scheduleId, schedule.seed_amount, schedule.seed_amount]
    );
    ...
    if (schedule.status === 'PENDING') {
      await pool.query(
        'UPDATE jackpot_schedules SET status = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2',
        ['ACTIVE', scheduleId]
      );
    }
   ...
   */
  async addContribution(contribution: JackpotContribution): Promise<void> {
    ...
    try {
      ...
      const scheduleResult = await client.query(
        'SELECT * FROM jackpot_schedules WHERE id = $1 AND status = $2',
        [contribution.schedule_id, 'ACTIVE']
      );
      ...
      let instanceResult = await client.query(
        'SELECT * FROM jackpot_instances WHERE schedule_id = $1 AND status = $2 ORDER BY created_at DESC LIMIT 1',
        [contribution.schedule_id, 'ACTIVE']
      );
      ...
      if (instanceResult.rows.length === 0) {
        ...
        instanceResult = await client.query(
          'SELECT * FROM jackpot_instances WHERE id = $1',
          [newInstanceId]
        );
      }
      ...
      await client.query(
        'UPDATE jackpot_instances SET current_size = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2',
        [newSize, instance.id]
      );
   ...
   */
  async triggerJackpotWin(instanceId: number, winnerId: number, winnerWalletName: string): Promise<number> {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
      ...
      const instanceResult = await client.query(
        'SELECT ji.*, js.name, js.currency_code, js.seed_amount FROM jackpot_instances ji JOIN jackpot_schedules js ON ji.schedule_id = js.id WHERE ji.id = $1 AND ji.status = $2',
        [instanceId, 'ACTIVE']
      );
      ...
      await client.query(
        'UPDATE jackpot_instances SET winner_user_id = $1, winner_wallet_name = $2, won_at = CURRENT_TIMESTAMP, status = $3, updated_at = CURRENT_TIMESTAMP WHERE id = $4',
        [winnerId, winnerWalletName, 'WON', instanceId]
      );
   ...
   */
  async getActiveJackpots(userId?: number, walletGroup?: string): Promise<any[]> {
    let query = `
   ...
   * Get jackpot history
   */
  async getJackpotHistory(filters?: { schedule_id?: number; limit?: number }): Promise<any[]> {
    let query = `
   ...
   */
  async updateJackpotSchedule(scheduleId: number, updates: Partial<JackpotSchedule>): Promise<void> {
    const fields: string[] = [];
    const values: any[] = [];
    let paramIndex = 1;
   ...
   */
  async deleteJackpotSchedule(scheduleId: number): Promise<void> {
    await pool.query(
      'UPDATE jackpot_schedules SET status = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2',
      ['DELETED', scheduleId]
    );
