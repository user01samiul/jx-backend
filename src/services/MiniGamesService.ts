...
interface MiniGameType {
  ...
  name: string;
  description: string;
  game_type: 'WHEEL' | 'MYSTERY_CHEST' | 'DICE' | 'SCRATCH_CARD' | 'COIN_FLIP';
  config: any;
  status: 'ACTIVE' | 'INACTIVE';
  play_cost_type?: 'FREE' | 'LOYALTY_POINTS' | 'CASH';
  play_cost_amount?: number;
...
interface MiniGamePrize {
  ...
  game_id: number;
  name: string;
  prize_type: 'CASH' | 'BONUS' | 'FREE_SPINS' | 'LOYALTY_POINTS' | 'NOTHING';
  prize_amount?: number;
  probability: number;
  display_config?: any;
  status: 'ACTIVE' | 'INACTIVE';
...
class MiniGamesService {
   ...
   */
  async getAllGameTypes(status?: string): Promise<any[]> {
   ...
   */
  async getGameTypeById(gameId: number): Promise<any> {
    const result = await pool.query(
      'SELECT * FROM mini_game_types WHERE id = $1',
      [gameId]
    );
   ...
   */
  async createGameType(game: MiniGameType): Promise<any> {
    const result = await pool.query(
      `INSERT INTO mini_game_types (
   ...
   */
  async updateGameType(gameId: number, updates: Partial<MiniGameType>): Promise<any> {
    const fields: string[] = [];
    const values: any[] = [];
    let paramIndex = 1;
    ...
    const result = await pool.query(
      `UPDATE mini_game_types SET ${fields.join(', ')}, updated_at = CURRENT_TIMESTAMP
   ...
   */
  async getGamePrizes(gameId: number, status?: string): Promise<any[]> {
   ...
   */
  async createPrize(prize: MiniGamePrize): Promise<any> {
    const result = await pool.query(
      `INSERT INTO mini_game_prizes (
   ...
   * Update prize (Admin)
   */
  async updatePrize(prizeId: number, updates: Partial<MiniGamePrize>): Promise<any> {
    const fields: string[] = [];
    const values: any[] = [];
    let paramIndex = 1;
    ...
    const result = await pool.query(
      `UPDATE mini_game_prizes SET ${fields.join(', ')}, updated_at = CURRENT_TIMESTAMP
       WHERE id = $${paramIndex} RETURNING *`,
      values
    );
   ...
   */
  async canPlayerPlay(userId: number, gameId: number): Promise<{ canPlay: boolean; reason?: string; nextAvailableAt?: Date }> {
    ...
    if (game.cooldown_minutes > 0) {
      const lastPlay = await pool.query(
        `SELECT MAX(played_at) as last_played FROM player_mini_game_plays
    ...
    if (game.max_plays_per_day) {
      ...
      const playsToday = await pool.query(
        `SELECT COUNT(*) as count FROM player_mini_game_plays
         WHERE user_id = $1 AND game_id = $2 AND played_at >= $3`,
        [userId, gameId, today]
      );
   ...
   */
  private async selectPrize(gameId: number): Promise<any> {
   ...
   * Play mini game
   */
  async playGame(userId: number, gameId: number): Promise<any> {
    ...
    try {
      ...
      if (game.play_cost_type === 'LOYALTY_POINTS' && game.play_cost_amount > 0) {
        const playerLoyalty = await client.query(
          'SELECT points FROM player_loyalty WHERE user_id = $1 FOR UPDATE',
          [userId]
        );
        ...
        await client.query(
          'UPDATE player_loyalty SET points = points - $1 WHERE user_id = $2',
          [game.play_cost_amount, userId]
        );
      ...
      const playResult = await client.query(
        `INSERT INTO player_mini_game_plays (user_id, game_id, prize_id, prize_awarded)
         VALUES ($1, $2, $3, $4)
         RETURNING *`,
        [userId, gameId, selectedPrize.id, selectedPrize.prize_type !== 'NOTHING']
      );
      ...
      if (selectedPrize.prize_type !== 'NOTHING') {
        switch (selectedPrize.prize_type) {
          case 'CASH':
            await client.query(
              'UPDATE users SET balance = balance + $1 WHERE id = $2',
              [selectedPrize.prize_amount, userId]
            );
            break;
          ...
          case 'LOYALTY_POINTS':
            await client.query(
              `UPDATE player_loyalty SET points = points + $1, lifetime_points = lifetime_points + $1
               WHERE user_id = $2`,
              [selectedPrize.prize_amount, userId]
            );
            ...
            await client.query(
              `INSERT INTO loyalty_point_transactions (user_id, points, transaction_type, reason, reference_id)
               VALUES ($1, $2, 'EARNED', 'Mini game prize', $3)`,
              [userId, selectedPrize.prize_amount, `minigame_${playResult.rows[0].id}`]
            );
            break;
      ...
      if (game.cooldown_minutes > 0) {
        ...
        await client.query(
          `INSERT INTO player_mini_game_spins (user_id, game_id, next_available_at)
           VALUES ($1, $2, $3)
           ON CONFLICT (user_id, game_id)
           DO UPDATE SET next_available_at = $3, last_spin_at = CURRENT_TIMESTAMP`,
          [userId, gameId, cooldownEnd]
        );
      }
   ...
   */
  async getPlayerPlays(userId: number, gameId?: number, limit: number = 50): Promise<any[]> {
    let query = `
   ...
   */
  async getPlayerCooldowns(userId: number): Promise<any[]> {
    const result = await pool.query(
      `SELECT pmgs.*, mgt.name as game_name, mgt.cooldown_minutes
       FROM player_mini_game_spins pmgs
       JOIN mini_game_types mgt ON pmgs.game_id = mgt.id
       WHERE pmgs.user_id = $1 AND pmgs.next_available_at > CURRENT_TIMESTAMP`,
      [userId]
    );
    return result.rows;
  }
   ...
   */
  async getGameStatistics(gameId: number): Promise<any> {
    ...
    const prizeStats = await pool.query(
      `SELECT mgp.name, mgp.prize_type, mgp.prize_amount, COUNT(*) as times_won
       FROM player_mini_game_plays pmgp
       JOIN mini_game_prizes mgp ON pmgp.prize_id = mgp.id
