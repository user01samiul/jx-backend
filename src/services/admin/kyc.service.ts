...
export class AdminKYCService {
  ...
  static async getPendingKYC(filters: KYCFiltersInput) {
  ...
  // Approve KYC
  static async approveKYC(userId: number, data: KYCVerificationInput) {
  ...
  // Reject KYC
  static async rejectKYC(userId: number, data: KYCVerificationInput) {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
  ...
  // Get KYC documents
  static async getKYCDocuments(userId?: number, filters?: KYCFiltersInput) {
    let whereConditions = ['1=1'];
    let values = [];
    let valueIndex = 1;
  ...
  // Verify KYC document
  static async verifyKYCDocument(data: KYCDocumentVerificationInput) {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
  ...
  static async createRiskAssessment(data: KYCRiskAssessmentInput) {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
  ...
  // Get KYC reports
  static async getKYCReports(filters: KYCComplianceReportInput) {
    ...
    if (include_details) {
      ...
      const detailsResult = await pool.query(detailsQuery, [start_date, end_date]);
