...
} from "../../api/admin/admin.support.schema";

export class AdminSupportService {
  
  // Get support tickets with filters
  static async getSupportTickets(filters: SupportTicketFiltersInput) {
  ...
  static async getSupportTicketById(id: number) {
    const query = `
      SELECT 
        st.*,
        u.username,
  ...
  static async updateSupportTicket(id: number, data: UpdateSupportTicketInput) {
  ...
  // Add reply to ticket
  static async addTicketReply(data: AddTicketReplyInput) {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
  ...
  static async createSupportCategory(data: CreateSupportCategoryInput) {
    const query = `
  ...
  static async getSupportStatistics(filters: SupportStatisticsFiltersInput) {
