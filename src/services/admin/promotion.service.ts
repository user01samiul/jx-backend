...
export class AdminPromotionService {
  ...
  static async getPromotions(filters: PromotionFiltersInput) {
  ...
  // Update promotion
  static async updatePromotion(id: number, data: UpdatePromotionInput) {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
  ...
  static async deletePromotion(id: number) {
    ...
    try {
      ...
      const claimsCheck = await client.query(
        'SELECT COUNT(*) FROM promotion_claims WHERE promotion_id = $1',
        [id]
      );
  ...
  static async togglePromotion(id: number) {
    const query = `
      UPDATE promotions 
  ...
  static async getPromotionStats(filters: PromotionStatsFiltersInput) {
  ...
  static async getPromotionClaims(promotionId: number, page: number = 1, limit: number = 20) {
    const offset = (page - 1) * limit;
    ...
    const countResult = await pool.query(countQuery, [promotionId]);
    const total = parseInt(countResult.rows[0].total);
    ...
    const dataResult = await pool.query(dataQuery, [promotionId, limit, offset]);
