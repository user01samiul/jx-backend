 ...
 */
export const getBetStatisticsService = async (timeRange: string = '7d') => {
 ...
 */
export const getBetAnalyticsService = async (
  timeRange: string = '7d',
  groupBy: string = 'day'
) => {
  const interval = getTimeRangeInterval(timeRange);
  ...
  let dateTrunc = 'day';
  if (groupBy === 'hour') dateTrunc = 'hour';
 ...
 */
export const getGamePerformanceService = async (
  timeRange: string = '7d',
  limit: number = 10
) => {
  const interval = getTimeRangeInterval(timeRange);
 ...
 */
export const getResultsDistributionService = async (timeRange: string = '7d') => {
  const interval = getTimeRangeInterval(timeRange);
 ...
 */
export const getProviderPerformanceService = async (
  timeRange: string = '7d',
  limit: number = 10
) => {
  const interval = getTimeRangeInterval(timeRange);
 ...
 */
export const getPlayerAnalyticsService = async (
  timeRange: string = '7d',
  limit: number = 10,
  sortBy: string = 'totalWagered',
  minBets: number = 10
) => {
  const interval = getTimeRangeInterval(timeRange);

  const playerAnalyticsResult = await pool.query(
    `
 ...
 */
export const getTimeAnalyticsService = async (
  timeRange: string = '7d',
  timezone: string = 'UTC'
) => {
  const interval = getTimeRangeInterval(timeRange);
  ...
  if (hourlyData.length === 0) {
    ...
    const mostBets = hourlyData.reduce((prev, current) =>
      current.totalBets > prev.totalBets ? current : prev
    , hourlyData[0]);
    const mostWagered = hourlyData.reduce((prev, current) =>
      current.totalWagered > prev.totalWagered ? current : prev
    , hourlyData[0]);
    const mostPlayers = hourlyData.reduce((prev, current) =>
      current.activePlayers > prev.activePlayers ? current : prev
    , hourlyData[0]);
