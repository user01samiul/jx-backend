...
export class PromotionService {
   ...
   */
  static async checkEligibility(userId: number, promotionId: number): Promise<PromotionEligibility> {
   ...
   */
  static async getWageringProgress(userId: number): Promise<WageringProgress[]> {
   ...
   */
  static async updateWageringProgress(userId: number, betAmount: number): Promise<void> {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');

      // Get active promotions with wagering requirements
      const activePromotionsResult = await client.query(
        `SELECT up.promotion_id, up.wagering_completed, p.wagering_requirement
         FROM user_promotions up
         JOIN promotions p ON up.promotion_id = p.id
         WHERE up.user_id = $1 AND up.status = 'active' AND p.wagering_requirement > 0`,
        [userId]
      ...
      for (const promotion of activePromotionsResult.rows) {
        ...
        await client.query(
          `UPDATE user_promotions 
           ...
           WHERE user_id = $2 AND promotion_id = $3`,
          [newWageringCompleted, userId, promotion.promotion_id]
        );
   ...
   */
  static async transferBonusToMain(userId: number, amount: number): Promise<boolean> {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
      ...
      await client.query(
        `INSERT INTO transactions (user_id, type, amount, balance_before, balance_after, currency, status, description, reference_id, created_at)
         VALUES ($1, 'adjustment', $2, $3, $4, 'USD', 'completed', $5, $6, CURRENT_TIMESTAMP)`,
        [userId, amount, currentMainBalance, currentMainBalance + amount, `Bonus transfer: $${amount}`, `bonus_transfer_${Date.now()}`]
      );
   ...
   */
  static async getBonusBalanceSummary(userId: number): Promise<any> {
    const result = await pool.query(
      `SELECT 
        ub.bonus_balance,
  ...
  private static async checkWelcomeBonusEligibility(client: any, userId: number, promotion: any): Promise<PromotionEligibility> {
  ...
  private static async checkDepositBonusEligibility(client: any, userId: number, promotion: any): Promise<PromotionEligibility> {
    // Check if user has made required deposits
    const depositResult = await client.query(
      "SELECT COALESCE(SUM(amount), 0) as total_deposits FROM transactions WHERE user_id = $1 AND type = 'deposit' AND status = 'completed'",
      [userId]
  ...
  private static async checkFreeSpinsEligibility(client: any, userId: number, promotion: any): Promise<PromotionEligibility> {
    // Check if user has made required deposits
    const depositResult = await client.query(
      "SELECT COALESCE(SUM(amount), 0) as total_deposits FROM transactions WHERE user_id = $1 AND type = 'deposit' AND status = 'completed'",
      [userId]
  ...
  private static async checkCashbackEligibility(client: any, userId: number, promotion: any): Promise<PromotionEligibility> {
    // For cashback, check if user has wagered enough
    const wageredResult = await client.query(
      "SELECT COALESCE(SUM(amount), 0) as total_wagered FROM transactions WHERE user_id = $1 AND type = 'bet' AND status = 'completed'",
      [userId]
    );
