...
export interface BonusTransaction {
  ...
  game_id: number | null;
  bet_id: number | null;
  wager_contribution: number | null;
...
export class BonusTransactionService {
  ...
  static async createTransaction(
    ...
    try {
      ...
      const walletResult = await client.query(
        'SELECT playable_bonus_balance FROM bonus_wallets WHERE player_id = $1',
        [data.player_id]
      );
   ...
   */
  static async getInstanceTransactions(
   ...
   */
  static async getPlayerStats(playerId: number): Promise<{
   ...
   */
  private static formatTransaction(row: any): BonusTransaction {
    return {
      id: row.id,
      bonus_instance_id: row.bonus_instance_id,
      player_id: row.player_id,
  ...
  static async getAuditLogs(options: {
    ...
    try {
      ...
      const countResult = await client.query(
        `SELECT COUNT(*) as total FROM bonus_audit_log WHERE ${whereClause}`,
        params
      );
