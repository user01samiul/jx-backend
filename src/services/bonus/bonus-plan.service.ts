...
export interface BonusPlan {
  ...
  award_type: 'flat_amount' | 'percentage';
  amount: number;
  currency?: string;
  wager_requirement_multiplier: number;
  wager_requirement_type?: string;
  wager_requirement_action?: string;
  is_incremental?: boolean;
  game_type_id?: number;
  description?: string;
  image_url?: string;
  is_playable?: boolean;
  playable_bonus_qualifies?: boolean;
  release_playable_winnings?: boolean;
  cancel_on_withdrawal?: boolean;
  max_trigger_all?: number;
  max_trigger_per_player?: number;
  min_bonus_threshold?: number;
  bonus_max_release?: number;
  recurrence_type?: string;
  allow_sportsbook?: boolean;
  allow_poker?: boolean;
  additional_award?: boolean;
  bonus_code?: string;
  max_code_usage?: number;
  current_code_usage?: number;
  loyalty_points_required?: number;
  vip_level_required?: number;
  cashback_percentage?: number;
  cashback_calculation_period?: string;
  status?: string;
  created_by?: number;
}

export class BonusPlanService {
  /**
   * Create a new bonus plan
   */
  static async createPlan(planData: BonusPlan, adminUserId: number): Promise<BonusPlan> {
    ...
    try {
      ...
      if (error.code === '23505' && error.constraint === 'bonus_plans_bonus_code_key') {
        throw new ApiError('Bonus code already exists', 400);
      }
      throw error;
    } finally {
      client.release();
  ...
  static async updatePlan(
    ...
    try {
      ...
      const oldResult = await client.query(
        'SELECT * FROM bonus_plans WHERE id = $1',
        [planId]
      );
   ...
   */
  static async getPlanById(planId: number): Promise<BonusPlan | null> {
    const client = await pool.connect();
  ...
  static async getPlans(filters: {
    ...
    limit?: number;
    offset?: number;
  } = {}): Promise<{ plans: BonusPlan[]; total: number }> {
    const client = await pool.connect();
    ...
    try {
      ...
      const countResult = await client.query(
        `SELECT COUNT(*) as total FROM bonus_plans ${whereClause}`,
        params
      );
   ...
   */
  static async getActiveDepositBonuses(depositAmount: number, paymentMethodId?: number): Promise<BonusPlan[]> {
    const client = await pool.connect();
    ...
    try {
      ...
      const params: any[] = [now, depositAmount];
   ...
   */
  static async getPlanByCode(code: string): Promise<BonusPlan | null> {
    const client = await pool.connect();
   ...
   */
  static async incrementCodeUsage(planId: number): Promise<void> {
    const client = await pool.connect();
   ...
   * Delete a bonus plan
   */
  static async deletePlan(planId: number, adminUserId: number): Promise<void> {
    const client = await pool.connect();
    ...
    try {
      ...
      if (parseInt(instancesResult.rows[0].count) > 0) {
        throw new ApiError('Cannot delete bonus plan with active instances', 400);
      }
   ...
   * Format plan data
   */
  private static formatPlan(row: any): BonusPlan {
    return {
      id: row.id,
      name: row.name,
   ...
   * Clone a bonus plan
   */
  static async clonePlan(
    planId: number,
    overrides: Partial<BonusPlan>,
    adminUserId: number
  ): Promise<BonusPlan> {
    const client = await pool.connect();
    ...
    try {
      ...
      delete (newPlanData as any).created_at;
      delete (newPlanData as any).updated_at;
   ...
   */
  static async getPlanAnalytics(planId: number): Promise<any> {
    const client = await pool.connect();
