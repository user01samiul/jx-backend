...
export interface BonusWalletInfo {
  ...
  releasable_amount: number; // Amount that can be transferred to main wallet (wagering complete)
  total_bonus_received: number;
  total_bonus_wagered: number;
  total_bonus_released: number;
  total_bonus_forfeited: number;
  total_bonus_transferred: number; // Lifetime transferred to main wallet
...
export class BonusWalletService {
    ...
    try {
      ...
      if (result.rows.length === 0) {
        ...
        result = await client.query(
          `INSERT INTO bonus_wallets (player_id, currency)
           VALUES ($1, $2)
           RETURNING *`,
          [playerId, currency]
        );
   ...
   */
  static async getBalance(playerId: number): Promise<BonusWalletInfo> {
    const client = await pool.connect();
   ...
   */
  static async deductBonus(playerId: number, amount: number): Promise<number> {
    const client = await pool.connect();
    ...
    try {
      ...
      if (wallet.playable_bonus_balance < amount) {
        throw new ApiError('Insufficient bonus balance', 400);
      }
   ...
   */
  static async addWinnings(playerId: number, amount: number): Promise<void> {
    const client = await pool.connect();
   ...
   */
  static async releaseToMainWallet(playerId: number, amount: number): Promise<void> {
    const client = await pool.connect();
   ...
   */
  static async forfeitBonus(playerId: number, amount: number): Promise<void> {
    const client = await pool.connect();
   ...
   */
  static async unlockBonus(playerId: number, amount: number): Promise<void> {
    const client = await pool.connect();
  ...
  static async transferToMainWallet(playerId: number, amount?: number): Promise<number> {
    ...
    try {
      ...
      if (releasableAmount <= 0) {
        await client.query('ROLLBACK');
        throw new ApiError('No releasable bonus funds available', 400);
      }
   ...
   * Format wallet data
   */
  private static formatWallet(row: any, releasableAmount: number = 0): BonusWalletInfo {
    return {
