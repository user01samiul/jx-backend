...
export class RFMSegmentationService {
   ...
   */
  static async calculateRFMScores(days: number = 90): Promise<RFMScore[]> {
    const query = `
      WITH rfm_base AS (
        SELECT
          u.id as user_id,
   ...
   */
  static determineSegment(r: number, f: number, m: number): string {
    for (const def of SEGMENT_DEFINITIONS) {
      if (
        r >= def.r_min && r <= def.r_max &&
   ...
   */
  static async saveRFMSegments(scores: RFMScore[]): Promise<void> {
   ...
   */
  static async getUsersBySegment(segment: string, limit: number = 100) {
    const query = `
      SELECT
        u.id,
        u.username,
    ...
    const result = await pool.query(query, [segment, limit]);
    return result.rows;
  }
   ...
   */
  static async recalculateAll(days: number = 90): Promise<{
    total_users: number;
    segments: any[];
  }> {
   ...
   */
  static getSegmentRecommendations(segment: string): string {
   ...
   */
  static async getSegmentHealthScore(): Promise<{
    health_score: number;
    distribution: any;
