...
import { cloudflareSolverService } from './cloudflare-solver.service';
...
class RotatingProxyService {
   ...
   */
  private async fetchProxiesFromSource(sourceUrl: string): Promise<string[]> {
    try {
  ...
  private async refreshProxyPool(): Promise<void> {
    ...
    const existingProxyMap = new Map(
      this.proxyPool.map(p => [`${p.host}:${p.port}`, p])
    );
   ...
   */
  private async getNextProxy(): Promise<ProxyInfo | null> {
   ...
   */
  private markProxyFailed(proxy: ProxyInfo): void {
    proxy.failCount++;
   ...
   */
  private markProxySuccess(proxy: ProxyInfo, responseTime: number): void {
    proxy.successCount++;
   ...
   */
  async makeRequest(targetUrl: string, config: AxiosRequestConfig = {}, maxRetries = 3): Promise<any> {
    ...
    try {
      const solution = await cloudflareSolverService.solveChallenge(targetUrl, 30000);
   ...
   */
  async testProxy(proxyUrl: string, testUrl = 'https://httpbin.org/ip'): Promise<boolean> {
    try {
...
// Singleton instance
export const rotatingProxyService = new RotatingProxyService();
