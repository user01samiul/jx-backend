...
class CloudflareSolverService {
   ...
   */
  async solveChallenge(targetUrl: string, maxTimeout = 30000): Promise<CloudflareSolution> {
   ...
   */
  private async waitForCloudflareChallengeSolved(page: any, maxTimeout: number): Promise<void> {
    const startTime = Date.now();
   ...
   */
  async makeRequestWithCookies(targetUrl: string, cookies: any[]): Promise<string> {
    const browser = await this.getBrowser();
    const page = await browser.newPage();
...
// Singleton instance
export const cloudflareSolverService = new CloudflareSolverService();

// Cleanup on process exit
process.on('SIGTERM', async () => {
  await cloudflareSolverService.cleanup();
});
