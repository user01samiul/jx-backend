...
interface PersonalJackpotConfig {
  id?: number;
  name: string;
  description: string;
  seed_amount: number;
  ...
  max_amount?: number;
  trigger_type: 'RANDOM' | 'SPIN_COUNT' | 'WAGER_AMOUNT' | 'TIME_BASED';
  trigger_config: any;
  game_ids?: number[];
  status: 'ACTIVE' | 'INACTIVE';
  vip_tier_required?: number;
}

interface PlayerJackpot {
  id?: number;
  user_id: number;
  config_id: number;
...
class PersonalJackpotsService {
   ...
   */
  async getAllConfigs(status?: string): Promise<any[]> {
   ...
   */
  async getConfigById(configId: number): Promise<any> {
    const result = await pool.query(
      'SELECT * FROM personal_jackpot_configs WHERE id = $1',
      [configId]
    );
   ...
   */
  async createConfig(config: PersonalJackpotConfig): Promise<any> {
    const result = await pool.query(
      `INSERT INTO personal_jackpot_configs (
   ...
   */
  async updateConfig(configId: number, updates: Partial<PersonalJackpotConfig>): Promise<any> {
    const fields: string[] = [];
    const values: any[] = [];
    let paramIndex = 1;
    ...
    const result = await pool.query(
      `UPDATE personal_jackpot_configs SET ${fields.join(', ')}, updated_at = CURRENT_TIMESTAMP
   ...
   */
  async initializePlayerJackpot(userId: number, configId: number): Promise<any> {
    ...
    const existing = await pool.query(
      `SELECT id FROM player_personal_jackpots
       WHERE user_id = $1 AND config_id = $2 AND status = 'ACTIVE'`,
      [userId, configId]
    );
   ...
   */
  async getPlayerJackpots(userId: number, status?: string): Promise<any[]> {
    let query = `
  ...
  async contributeToJackpot(
    ...
    try {
      ...
      let jackpot = await client.query(
        `SELECT ppj.*, pjc.increment_percentage, pjc.max_amount, pjc.game_ids
         FROM player_personal_jackpots ppj
         JOIN personal_jackpot_configs pjc ON ppj.config_id = pjc.id
   ...
   */
  async checkTrigger(userId: number, jackpotId: number): Promise<{ triggered: boolean; reason?: string }> {
    const jackpot = await pool.query(
      `SELECT ppj.*, pjc.trigger_type, pjc.trigger_config
       FROM player_personal_jackpots ppj
       JOIN personal_jackpot_configs pjc ON ppj.config_id = pjc.id
       WHERE ppj.id = $1 AND ppj.user_id = $2`,
      [jackpotId, userId]
    );
   ...
   * Trigger jackpot win
   */
  async triggerJackpotWin(userId: number, jackpotId: number): Promise<any> {
    const client = await pool.connect();
    ...
    try {
      ...
      // Record win
      await client.query(
        `INSERT INTO personal_jackpot_wins (jackpot_id, user_id, win_amount)
         VALUES ($1, $2, $3)`,
        [jackpotId, userId, jackpot.current_amount]
      );

      // Credit player's balance
      await client.query(
        'UPDATE users SET balance = balance + $1 WHERE id = $2',
        [jackpot.current_amount, userId]
      );
   ...
   */
  async getPlayerWins(userId: number, limit: number = 50): Promise<any[]> {
    const result = await pool.query(
      `SELECT pjw.*, ppj.config_id, pjc.name as jackpot_name
   ...
   */
  async getJackpotStatistics(configId: number): Promise<any> {
   ...
   */
  async autoInitializeJackpots(): Promise<void> {
    // Get all active configs with auto-assign enabled
    const configs = await pool.query(
      `SELECT * FROM personal_jackpot_configs WHERE status = 'ACTIVE'`
    );
...
export default new PersonalJackpotsService();
