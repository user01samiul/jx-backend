 ...
 * @access Admin
 */
router.get('/schedules', authenticate, adminAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (wallet_group) filters.wallet_group = wallet_group;

    const schedules = await JackpotService.getJackpotSchedules(filters);
 ...
 * @access Admin
 */
router.post('/schedules', authenticate, adminAuth, async (req: Request, res: Response) => {
  try {
    const scheduleData = req.body;
    const scheduleId = await JackpotService.createJackpotSchedule(scheduleData);
 ...
 * @access Admin
 */
router.put('/schedules/:id', authenticate, adminAuth, async (req: Request, res: Response) => {
  try {
    const scheduleId = parseInt(req.params.id);
    const updates = req.body;
    await JackpotService.updateJackpotSchedule(scheduleId, updates);
 ...
 * @access Admin
 */
router.delete('/schedules/:id', authenticate, adminAuth, async (req: Request, res: Response) => {
  try {
    const scheduleId = parseInt(req.params.id);
    await JackpotService.deleteJackpotSchedule(scheduleId);
 ...
 * @access Admin
 */
router.post('/instances/start/:scheduleId', authenticate, adminAuth, async (req: Request, res: Response) => {
  try {
    const scheduleId = parseInt(req.params.scheduleId);
    const instanceId = await JackpotService.startJackpotInstance(scheduleId);
 ...
 * @access Admin
 */
router.post('/instances/trigger-win', authenticate, adminAuth, async (req: Request, res: Response) => {
  try {
    ...
    const winAmount = await JackpotService.triggerJackpotWin(instanceId, userId, walletName);
...
router.get('/history', async (req: Request, res: Response) => {
  try {
    ...
    const history = await JackpotService.getJackpotHistory(filters);
 ...
 * @access Admin
 */
router.post('/sync', authenticate, adminAuth, async (req: Request, res: Response) => {
  try {
 ...
 * @access Admin
 */
router.post('/update-amount', authenticate, adminAuth, async (req: Request, res: Response) => {
  try {
    ...
    const scheduleResult = await pool.query(
      'UPDATE jackpot_schedules SET current_amount = $1, updated_at = CURRENT_TIMESTAMP WHERE innova_schedule_id = $2 RETURNING id, name',
      [current_amount, innova_schedule_id]
    );
    ...
    await pool.query(
      'UPDATE jackpot_instances SET current_amount = $1, updated_at = CURRENT_TIMESTAMP WHERE schedule_id = $2 AND status = $3',
      [current_amount, scheduleResult.rows[0].id, 'ACTIVE']
    );
