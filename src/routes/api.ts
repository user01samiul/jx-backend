...
router.post("/payment/create", authenticate, async (req, res) => {
  try {
    ...
    if (!gateway || (gateway && gateway.code === 'vimplay')) {
      const vimplayResult = await pool.query(
        `SELECT * FROM game_provider_configs WHERE id = $1 AND LOWER(provider_name) = 'vimplay'`,
        [gateway_id]
      );
    ...
    const transactionResult = await pool.query(
      `INSERT INTO transactions (user_id, type, amount, currency, status, description, reference_id, external_reference, metadata)
 ...
 */
router.get("/payment/crypto-conversion", authenticate, async (req, res) => {
  try {
    const { getCryptoConversion } = require("../api/payment/payment.controller");
    await getCryptoConversion(req, res, (error: any) => {
      if (error) {
