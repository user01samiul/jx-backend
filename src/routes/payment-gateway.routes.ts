 ...
 */
router.post("/payment-gateways", validateRequest({ body: CreatePaymentGatewayInput }), async (req, res) => {
  try {
 ...
 *         description: Unauthorized
 */
router.get("/payment-gateways", validateRequest({ query: PaymentGatewayFiltersInput }), async (req, res) => {
  try {
 ...
 *         description: Unauthorized
 */
router.put("/payment-gateways/:id", validateRequest({ body: UpdatePaymentGatewayInput }), async (req, res) => {
  try {
    const gatewayId = parseInt(req.params.id);
    if (isNaN(gatewayId)) {
      res.status(400).json({ success: false, message: "Invalid gateway ID" });
