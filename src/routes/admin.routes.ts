...
import {
  ...
  BulkCategoryOperationSchema,
  CategoryStatsFiltersSchema
...
import {
  ...
  PromotionStatsFiltersSchema
} from "../api/admin/admin.promotion.schema";
import {
...
import { updateUserStatusService } from "../services/admin/admin.service";
import { getDashboardStats, getRealtimeStats } from "../controllers/dashboardStatsController";
import {
  // Provider Management
  getProviders,
  createProvider,
  ...
  startCronJobs,
  stopCronJobs,
  triggerManualAutoAdjustment,
  triggerManualDailySummary
} from "../api/admin/admin.controller";
import {
  createPromotion,
  getPromotions,
...
import {
  resetSystem,
  getResetStats
} from "../api/admin/system-reset.controller";
import {
...
import { 
  AddProviderConfigSchema, 
  ImportGamesByCategorySchema, 
  ImportGameByIdSchema 
} from "../api/admin/admin.game-import.schema";
import { AdminGameImportService } from "../services/admin/admin.game-import.service";
import { 
  getGamesWithStatus,
  updateGameStatusById,
  updateGameStatusByCategory,
  updateGameStatusByProvider,
  bulkUpdateGameStatus,
  getGameStatusStats,
  getRecentStatusChanges
} from "../api/admin/admin.game-management.controller";
import { 
  GameStatusFiltersSchema,
  UpdateGameStatusByIdSchema,
  UpdateGameStatusByCategorySchema,
  UpdateGameStatusByProviderSchema,
  BulkUpdateGameStatusSchema
} from "../api/admin/admin.game-management.schema";
import {
  getSettings as getGGRFilterSettings,
  updateSettings as updateGGRFilterSettings,
  getAuditLogs as getGGRAuditLogs,
  getGGRReportSummary
} from "../services/provider/ggr-filter.service";
import {
  createCategory,
  getCategories,
  getCategoryById,
...
import {
  ...
  deleteModule,
  getModuleHierarchy
} from "../api/admin/modules/modules.controller";
import { 
  CreateModuleSchema, 
  UpdateModuleSchema, 
  ModuleQuerySchema 
} from "../api/admin/modules/modules.schema";
import { getRtpTargetProfit, setRtpTargetProfit, setRtpAutoAdjustment } from "../rtpAuto";
import { AdminCreateUserInput } from "../api/admin/admin.schema";
import { createUser } from "../api/admin/admin.controller";
...
router.get("/modules", validate({ query: ModuleQuerySchema }), getAllModules);
 ...
 */
router.get("/dashboard/stats/realtime", authenticate, authorize(["Admin", "Manager"]), getRealtimeStats);
 ...
 */
router.get("/games", authenticate, authorize(["Admin"]), getGamesForAdmin);
 ...
 */
router.get("/games/status", validate({ query: GameStatusFiltersSchema }), getGamesWithStatus);
 ...
 *         description: Internal server error
 */
router.put("/games/status/id", validate({ body: UpdateGameStatusByIdSchema }), updateGameStatusById);
 ...
 *         description: Internal server error
 */
router.put("/games/status/category", validate({ body: UpdateGameStatusByCategorySchema }), updateGameStatusByCategory);
 ...
 *         description: Internal server error
 */
router.put("/games/status/provider", validate({ body: UpdateGameStatusByProviderSchema }), updateGameStatusByProvider);
 ...
 *         description: Internal server error
 */
router.put("/games/status/bulk", validate({ body: BulkUpdateGameStatusSchema }), bulkUpdateGameStatus);
 ...
 *         description: Internal server error
 */
router.get("/games/status/stats", getGameStatusStats);
 ...
 *         description: Internal server error
 */
router.get("/games/status/changes", getRecentStatusChanges);
 ...
 */
router.post("/games", validate({ body: CreateGameInput }), createGame);
 ...
 */
router.put("/games/:id", validate({ body: UpdateGameInput }), updateGame);
 ...
 */
router.get("/users/search", authenticate, authorize(["Admin", "Manager", "Support"]), async (req, res) => {
  try {
...
router.get("/users/:id/roles", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT r.id, r.name, r.description, r.permissions, ur.created_at as assigned_at
       FROM user_roles ur
 ...
 *         description: Internal server error
 */
router.post("/users", validate({ body: AdminCreateUserInput }), createUser);
 ...
 */
router.put("/users/:id/status", validate({ body: UpdateUserStatusInput }), updateUserStatus);
 ...
 */
router.put("/users/:id/balance", validate({ body: UpdateUserBalanceInput }), updateUserBalance);
 ...
 */
router.post("/users/:id/disable", authenticate, authorize(["Admin"]), async (req, res) => {
  try {
    const userId = parseInt(req.params.id);
    if (isNaN(userId)) {
 ...
 *         description: User not found
 */
router.post("/users/:id/enable", authenticate, authorize(["Admin"]), async (req, res) => {
  try {
    const userId = parseInt(req.params.id);
    if (isNaN(userId)) {
      return res.status(400).json({ success: false, message: "Invalid user ID" });
 ...
 *         description: User not found
 */
router.post("/users/:id/ban", authenticate, authorize(["Admin"]), async (req, res) => {
  try {
    const userId = parseInt(req.params.id);
    if (isNaN(userId)) {
      return res.status(400).json({ success: false, message: "Invalid user ID" });
...
const gameImportService = new AdminGameImportService();
...
router.post(
  ...
  authenticate,
  adminAuth,
  validate({ body: AddProviderConfigSchema }),
  async (req, res) => {
...
router.post(
  ...
  authenticate,
  adminAuth,
  validate({ body: ImportGamesByCategorySchema }),
  async (req, res) => {
...
router.post(
  ...
  authenticate,
  adminAuth,
  validate({ body: ImportGameByIdSchema }),
  async (req, res) => {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const result = await new AdminGameImportService().importAllGamesByProvider(provider_name, !!debug);
      res.json(result);
    } catch (error) {
 ...
 */
router.post("/payment-gateways", validate({ body: CreatePaymentGatewayInput }), async (req, res) => {
  try {
 ...
 *         description: Unauthorized
 */
router.get("/payment-gateways", validate({ query: PaymentGatewayFiltersInput }), async (req, res) => {
  try {
 ...
 *         description: Unauthorized
 */
router.put("/payment-gateways/:id", validate({ body: UpdatePaymentGatewayInput }), async (req, res) => {
  try {
    const gatewayId = parseInt(req.params.id);
    if (isNaN(gatewayId)) {
      res.status(400).json({ success: false, message: "Invalid gateway ID" });
...
router.get("/ggr-filter", async (req, res) => {
  try {
    const settings = await getGGRFilterSettings();
...
router.put("/ggr-filter", async (req, res) => {
  try {
    ...
    const settings = await updateGGRFilterSettings(filter_percent, tolerance);
    res.status(200).json({ success: true, data: settings });
  } catch (error: any) {
    res.status(500).json({ success: false, message: error.message });
  }
...
router.get("/ggr-audit-logs", async (req, res) => {
  try {
    ...
    const logs = await getGGRAuditLogs(limit, offset);
...
router.get("/ggr-report-summary", async (req, res) => {
  try {
    ...
    const summary = await getGGRReportSummary(startDate as string, endDate as string);
 ...
 *         description: Internal server error
 */
router.get("/modules/hierarchy", getModuleHierarchy);
 ...
 *         description: Internal server error
 */
router.post("/modules", validate({ body: CreateModuleSchema }), createModule);
 ...
 *         description: Internal server error
 */
router.put("/modules/:id", validate({ body: UpdateModuleSchema }), updateModule);
 ...
 *         description: Forbidden - Admin access required
 */
router.get("/rtp/settings", getRTPSettings);
 ...
 *         description: Forbidden - Admin access required
 */
router.put("/rtp/settings", validate({ body: UpdateRTPSettingsInput }), updateRTPSettings);
 ...
 *         description: Forbidden - Admin access required
 */
router.get("/rtp/analytics", validate({ query: RTPAnalyticsFiltersInput }), getRTPAnalytics);
 ...
 *         description: Forbidden - Admin access required
 */
router.post("/rtp/bulk-update", validate({ body: BulkUpdateRTPInput }), bulkUpdateRTP);
 ...
 *         description: Forbidden - Admin access required
 */
router.get("/rtp/report", validate({ query: RTPReportFiltersInput }), getRTPReport);
 ...
 *         description: Internal server error
 */
router.post("/bets/:id/cancel", authenticate, authorize(["Admin"]), async (req, res) => {
  try {
    ...
    const result = await cancelGameService(
      bet.user_id, 
      transaction.external_reference || transaction.id.toString(), // Use external_reference if available
      bet.game_id, 
 ...
 *         description: Invalid input
 */
router.get("/analytics/profit", authenticate, authorize(["Admin"]), async (req, res) => {
  ...
  const pool = require("../db/postgres").default;
  const result = await pool.query(
    `SELECT SUM(bet_amount) as total_bets, SUM(win_amount) as total_wins FROM bets WHERE placed_at BETWEEN $1 AND $2`,
    [start_date, end_date]
  );
 ...
 *                   example: manual
 */
router.get("/rtp/target-profit", getRtpTargetProfit);
 ...
 *                   example: manual
 */
router.post("/rtp/target-profit", setRtpTargetProfit);
 ...
 *                   example: auto
 */
router.post("/rtp/auto-adjustment", setRtpAutoAdjustment);
 ...
 *         description: Forbidden - Admin access required
 */
router.post("/profit/auto-adjustment", triggerAutoAdjustment);
 ...
 *         description: Forbidden - Admin access required
 */
router.get("/profit/performance", getProfitPerformance);
 ...
 *         description: Forbidden - Admin access required
 */
router.post("/cron/trigger-auto-adjustment", triggerManualAutoAdjustment);
 ...
 *         description: Forbidden - Admin access required
 */
router.post("/cron/trigger-daily-summary", triggerManualDailySummary);
 ...
 *         description: Forbidden - Admin access required
 */
router.get("/promotions", authenticate, authorize(["Admin"]), validate({ query: PromotionFiltersSchema }), getPromotions);
 ...
 *         description: Forbidden - Admin access required
 */
router.post("/promotions", authenticate, authorize(["Admin"]), validate({ body: CreatePromotionSchema }), createPromotion);
 ...
 *         description: Forbidden - Admin access required
 */
router.get("/promotions/:id", authenticate, authorize(["Admin"]), getPromotionById);
 ...
 *         description: Forbidden - Admin access required
 */
router.put("/promotions/:id", authenticate, authorize(["Admin"]), validate({ body: UpdatePromotionSchema }), updatePromotion);
 ...
 *         description: Forbidden - Admin access required
 */
router.patch("/promotions/:id/toggle", authenticate, authorize(["Admin"]), validate({ body: TogglePromotionSchema }), togglePromotion);
 ...
 *         description: Forbidden - Admin access required
 */
router.get("/promotions/stats", authenticate, authorize(["Admin"]), validate({ query: PromotionStatsFiltersSchema }), getPromotionStats);
 ...
 *         description: Forbidden - Admin access required
 */
router.get("/promotions/stats/overview", authenticate, authorize(["Admin"]), getPromotionOverviewStats);
 ...
 *         description: Forbidden - Admin access required
 */
router.get("/promotions/:id/claims", authenticate, authorize(["Admin"]), getPromotionClaims);
 ...
 *         description: Forbidden - Admin access required
 */
router.get("/kyc/pending", authenticate, authorize(["Admin"]), validate({ query: KYCFiltersSchema }), getPendingKYC);

// Get all KYC submissions (no status filter)
router.get("/kyc/submissions", authenticate, authorize(["Admin"]), validate({ query: KYCFiltersSchema }), getAllKYCSubmissions);

// Get approved KYC submissions
router.get("/kyc/approved", authenticate, authorize(["Admin"]), validate({ query: KYCFiltersSchema }), getApprovedKYC);

// Get rejected KYC submissions
router.get("/kyc/rejected", authenticate, authorize(["Admin"]), validate({ query: KYCFiltersSchema }), getRejectedKYC);

// Get user info by ID (for frontend)
router.get("/kyc/users/:user_id", authenticate, authorize(["Admin"]), getKYCUserInfo);
 ...
 *         description: Forbidden - Admin access required
 */
router.get("/kyc/:user_id", authenticate, authorize(["Admin"]), getKYCByUserId);
 ...
 *         description: Forbidden - Admin access required
 */
router.get("/kyc/documents", authenticate, authorize(["Admin"]), getKYCDocuments);
 ...
 *         description: Forbidden - Admin access required
 */
router.put("/kyc/documents/:document_id/verify", authenticate, authorize(["Admin"]), validate({ body: KYCDocumentVerificationSchema }), verifyKYCDocument);
 ...
 *         description: Forbidden - Admin access required
 */
router.get("/kyc/reports", authenticate, authorize(["Admin"]), validate({ query: KYCComplianceReportSchema }), getKYCReports);
 ...
 *         description: Forbidden - Admin access required
 */
router.get("/kyc/audit-logs", authenticate, authorize(["Admin"]), getKYCAuditLogs);
 ...
 */
router.post("/system/reset", authenticate, authorize(["Admin"]), resetSystem);
 ...
 *         description: Forbidden - admin privileges required
 */
router.get("/system/reset/stats", authenticate, authorize(["Admin"]), getResetStats);
 ...
 *         description: Internal server error
 */
router.post("/users/bulk-status", authenticate, authorize(["Admin"]), async (req, res) => {
  try {
    ...
    const statusData = {
      status: status as "Active" | "Inactive" | "Suspended" | "Banned",
 ...
 *         description: User not found
 */
router.post("/users/:id/blacklist", authenticate, authorize(["Admin"]), async (req, res) => {
  try {
    const userId = parseInt(req.params.id);
    if (isNaN(userId)) {
      return res.status(400).json({ success: false, message: "Invalid user ID" });
 ...
 *         description: User not found
 */
router.post("/users/:id/unblacklist", authenticate, authorize(["Admin"]), async (req, res) => {
  try {
    const userId = parseInt(req.params.id);
    if (isNaN(userId)) {
      return res.status(400).json({ success: false, message: "Invalid user ID" });
 ...
 *         description: Internal server error
 */
router.get("/users/blacklist", authenticate, authorize(["Admin"]), async (req, res) => {
  try {
...
router.get("/analytics/player-behavior/:user_id", authenticate, authorize(["Admin"]), getPlayerBehavior);
router.post("/analytics/player-behavior/:user_id/calculate", authenticate, authorize(["Admin"]), calculateBehaviorScores);
router.get("/analytics/player-behavior/top-engaged", authenticate, authorize(["Admin"]), getTopEngagedPlayers);
router.get("/analytics/player-behavior/heatmap", authenticate, authorize(["Admin"]), getSessionHeatmap);

// RFM Segmentation
router.get("/analytics/rfm/segments", authenticate, authorize(["Admin"]), getRFMSegments);
router.get("/analytics/rfm/segments/:segment/users", authenticate, authorize(["Admin"]), getUsersBySegment);
router.post("/analytics/rfm/recalculate", authenticate, authorize(["Admin"]), recalculateRFM);
router.get("/analytics/rfm/health", authenticate, authorize(["Admin"]), getRFMHealthScore);

// Churn Prediction
router.get("/analytics/churn/prediction/:user_id", authenticate, authorize(["Admin"]), getChurnPrediction);
router.get("/analytics/churn/high-risk", authenticate, authorize(["Admin"]), getHighRiskUsers);
router.get("/analytics/churn/statistics", authenticate, authorize(["Admin"]), getChurnStatistics);
router.post("/analytics/churn/run-workflow", authenticate, authorize(["Admin"]), runChurnPredictionWorkflow);

// Session Tracking (for frontend - requires authentication but available to all authenticated users)
router.post("/analytics/session/start", authenticate, startSession);
router.post("/analytics/session/end", authenticate, endSession);
router.post("/analytics/event/track", authenticate, trackEvent);
 ...
 *         description: Server error
 */
router.get("/vip/tiers", authenticate, authorize(["Admin", "Manager", "Support"]), async (req, res) => {
  try {
    const pool = require('../db/postgres').default;
 ...
 *         description: Server error
 */
router.get("/vip/players", authenticate, authorize(["Admin", "Manager", "Support"]), async (req, res) => {
  try {
 ...
 *         description: Server error
 */
router.get("/vip/stats", authenticate, authorize(["Admin", "Manager", "Support"]), async (req, res) => {
  try {
    const pool = require('../db/postgres').default;
