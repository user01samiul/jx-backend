 ...
 * @access Admin only
 */
router.get('/', auth_middleware_1.authenticate, admin_middleware_1.adminMiddleware, async (req, res) => {
    try {
 ...
 * @access Admin only
 */
router.get('/stats', auth_middleware_1.authenticate, admin_middleware_1.adminMiddleware, async (req, res) => {
    try {
        const statsQuery = `
      SELECT
    ...
    `;
        const result = await postgres_1.default.query(statsQuery);
 ...
 * @access Admin only
 */
router.get('/:campaignCode', auth_middleware_1.authenticate, admin_middleware_1.adminMiddleware, async (req, res) => {
    try {
    ...
    `;
        const result = await postgres_1.default.query(query, [campaignCode]);
 ...
 * @access Admin only
 */
router.post('/:campaignCode/cancel', auth_middleware_1.authenticate, admin_middleware_1.adminMiddleware, async (req, res) => {
    try {
        const { campaignCode } = req.params;
        // Update campaign status to cancelled
        const result = await postgres_1.default.query(`UPDATE user_free_spins_campaigns
       SET status = 'cancelled', updated_at = CURRENT_TIMESTAMP
       WHERE campaign_code = $1 AND status IN ('pending', 'active')
       RETURNING *`, [campaignCode]);
        if (result.rows.length === 0) {
            return res.status(404).json({
                success: false,
