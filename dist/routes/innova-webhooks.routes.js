 ...
 */
async function handleNewJackpotInstance(data) {
    const { id: innova_instance_id, jackpot: innova_schedule_id, name, size, seed, currency, timestamp } = data;
    // Check if schedule exists in our database
    let scheduleResult = await postgres_1.default.query('SELECT id FROM jackpot_schedules WHERE innova_schedule_id = $1', [innova_schedule_id]);
    let scheduleId;
    if (scheduleResult.rows.length === 0) {
        // Create new schedule if it doesn't exist
        const newSchedule = await postgres_1.default.query(`INSERT INTO jackpot_schedules (
        innova_schedule_id, name, currency_code, seed_amount,
        current_amount, status, type
      ) VALUES ($1, $2, $3, $4, $5, 'ACTIVE', 'CASINO')
      RETURNING id`, [innova_schedule_id, name, currency, seed, size]);
    ...
    const existingInstance = await postgres_1.default.query('SELECT id FROM jackpot_instances WHERE innova_instance_id = $1', [innova_instance_id]);
    if (existingInstance.rows.length === 0) {
        // Create new jackpot instance
        await postgres_1.default.query(`INSERT INTO jackpot_instances (
        schedule_id, innova_instance_id, current_amount, seed_size, status, started_at
      ) VALUES ($1, $2, $3, $4, 'ACTIVE', $5)`, [scheduleId, innova_instance_id, size, seed, new Date(timestamp)]);
        // Update schedule current amount
        await postgres_1.default.query('UPDATE jackpot_schedules SET current_amount = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [size, scheduleId]);
 ...
 */
async function handleJackpotWin(data) {
    const { id: innova_instance_id, name, size: winAmount, currency, winner_entity: winnerId, winner: walletName, timestamp } = data;
    // Find jackpot instance
    const instanceResult = await postgres_1.default.query(`SELECT ji.id, ji.schedule_id, js.name as schedule_name
     FROM jackpot_instances ji
     JOIN jackpot_schedules js ON ji.schedule_id = js.id
     WHERE ji.innova_instance_id = $1`, [innova_instance_id]);
    ...
    // Record jackpot win
    await postgres_1.default.query(`INSERT INTO jackpot_winners (
      instance_id, user_id, wallet_name, win_amount, won_at
    ) VALUES ($1, $2, $3, $4, $5)`, [instance.id, winnerId, walletName, winAmount, new Date(timestamp)]);
    // Update instance status
    await postgres_1.default.query('UPDATE jackpot_instances SET status = $1, ended_at = $2 WHERE id = $3', ['FINISHED', new Date(timestamp), instance.id]);
 ...
 */
async function handleJackpotSizeUpdate(data) {
    const { id: innova_instance_id, size: currentAmount, progress, timestamp } = data;
    // Update jackpot instance current amount
    const result = await postgres_1.default.query(`UPDATE jackpot_instances ji
     SET current_amount = $1, updated_at = $2
     FROM jackpot_schedules js
     WHERE ji.innova_instance_id = $3 AND ji.schedule_id = js.id
     RETURNING js.id as schedule_id`, [currentAmount, new Date(timestamp), innova_instance_id]);
    if (result.rows.length > 0) {
        // Also update schedule current amount
        await postgres_1.default.query('UPDATE jackpot_schedules SET current_amount = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [currentAmount, result.rows[0].schedule_id]);
 ...
 */
async function handleNewTournamentInstance(data) {
    const { id: innova_instance_id, tournament: innova_schedule_id, name, currency, status, start, end, timestamp } = data;
    // Check if schedule exists
    let scheduleResult = await postgres_1.default.query('SELECT id FROM tournament_schedules WHERE innova_schedule_id = $1', [innova_schedule_id]);
    let scheduleId;
    if (scheduleResult.rows.length === 0) {
        // Create new schedule
        const newSchedule = await postgres_1.default.query(`INSERT INTO tournament_schedules (
        innova_schedule_id, name, currency_code, status,
        start_time, end_time, prize_pool, min_bet
      ) VALUES ($1, $2, $3, $4, $5, $6, 0, 0)
      RETURNING id`, [innova_schedule_id, name, currency, status, new Date(start), new Date(end)]);
        scheduleId = newSchedule.rows[0].id;
    }
    else {
        scheduleId = scheduleResult.rows[0].id;
    ...
    // Check if instance already exists
    const existingInstance = await postgres_1.default.query('SELECT id FROM tournament_instances WHERE innova_instance_id = $1', [innova_instance_id]);
    if (existingInstance.rows.length === 0) {
        // Create new tournament instance
        await postgres_1.default.query(`INSERT INTO tournament_instances (
        schedule_id, innova_instance_id, status, started_at
      ) VALUES ($1, $2, $3, $4)`, [scheduleId, innova_instance_id, status, new Date(timestamp)]);
 ...
 */
async function handleTournamentStatusUpdate(data) {
    const { id: innova_instance_id, status, previousStatus, timestamp } = data;
    // Update tournament instance status
    const result = await postgres_1.default.query(`UPDATE tournament_instances ti
     SET status = $1, updated_at = $2
     FROM tournament_schedules ts
     WHERE ti.innova_instance_id = $3 AND ti.schedule_id = ts.id
     RETURNING ts.id as schedule_id, ts.name`, [status, new Date(timestamp), innova_instance_id]);
    if (result.rows.length > 0) {
        // Also update schedule status
        await postgres_1.default.query('UPDATE tournament_schedules SET status = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [status, result.rows[0].schedule_id]);
