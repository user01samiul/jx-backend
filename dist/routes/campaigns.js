...
const innova_campaigns_service_1 = __importDefault(require("../services/provider/innova-campaigns.service"));
...
router.get('/vendors', auth_middleware_1.authenticate, auth_middleware_1.adminAuth, async (req, res) => {
    try {
        const vendors = await innova_campaigns_service_1.default.listVendors();
...
router.get('/game-limits', auth_middleware_1.authenticate, auth_middleware_1.adminAuth, async (req, res) => {
    ...
    const vendorsArray = vendors ? vendors.split(',') : undefined;
    const gamesArray = games ? games.split(',').map(Number) : undefined;
    const currenciesArray = currencies ? currencies.split(',') : undefined;
    try {
        const limits = await innova_campaigns_service_1.default.getGameLimits(vendorsArray, gamesArray, currenciesArray);
 ...
 * @access Admin
 */
router.get('/games', auth_middleware_1.authenticate, auth_middleware_1.adminAuth, async (req, res) => {
    try {
...
router.get('/', auth_middleware_1.authenticate, auth_middleware_1.adminAuth, async (req, res) => {
    ...
    `;
      ...
      LIMIT ${per_page ? parseInt(per_page) : 100}
    `;
        const result = await postgres_1.default.query(query, params);
...
router.get('/:campaignCode', auth_middleware_1.authenticate, auth_middleware_1.adminAuth, async (req, res) => {
    try {
        ...
        const campaign = await innova_campaigns_service_1.default.getCampaignDetails(campaignCode);
...
router.post('/', auth_middleware_1.authenticate, auth_middleware_1.adminAuth, async (req, res) => {
    try {
        ...
        try {
            ...
            await innova_campaigns_service_1.default.createCampaign(campaignData);
        ...
        try {
            ...
            const campaignInsertResult = await client.query(`INSERT INTO campaigns (
        ...
        RETURNING id`, [
            ...
            const players = Array.isArray(campaignData.players)
                ...
                : (campaignData.players ? campaignData.players.split(',').map((p) => p.trim()) : []);
...
router.post('/:campaignCode/cancel', auth_middleware_1.authenticate, auth_middleware_1.adminAuth, async (req, res) => {
    try {
        const { campaignCode } = req.params;
        await innova_campaigns_service_1.default.cancelCampaign({ campaign_code: campaignCode });
...
router.post('/:campaignCode/players/add', auth_middleware_1.authenticate, auth_middleware_1.adminAuth, async (req, res) => {
    try {
        ...
        await innova_campaigns_service_1.default.addPlayers({ campaign_code: campaignCode, players });
...
router.post('/:campaignCode/players/remove', auth_middleware_1.authenticate, auth_middleware_1.adminAuth, async (req, res) => {
    try {
        ...
        await innova_campaigns_service_1.default.removePlayers({ campaign_code: campaignCode, players });
 ...
 * @access Admin
 */
router.post('/:campaignCode/players/add-all', auth_middleware_1.authenticate, auth_middleware_1.adminAuth, async (req, res) => {
    try {
        const { campaignCode } = req.params;
        ...
        const result = await postgres_1.default.query('SELECT id FROM users WHERE status_id = 1 ORDER BY id ASC');
        ...
        const campaignResult = await postgres_1.default.query(`SELECT c.*, cg.game_id, cg.total_bet
       FROM campaigns c
       LEFT JOIN campaign_games cg ON c.id = cg.campaign_id
       ...
       LIMIT 1`, [campaignCode]);
        ...
        try {
            await innova_campaigns_service_1.default.addPlayers({
 ...
 * @access Player
 */
router.get('/user/me/debug', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user.userId || req.user.id; // Support both userId and id
        // Get ALL campaigns for this user without filters
        const allCampaigns = await postgres_1.default.query(`SELECT id, campaign_code, status, begins_at, expires_at, freespins_remaining, created_at
       FROM user_free_spins_campaigns
       WHERE user_id = $1
       ORDER BY created_at DESC`, [userId]);
        // Get campaigns with filters
        const filteredCampaigns = await postgres_1.default.query(`SELECT id, campaign_code, status, begins_at, expires_at, freespins_remaining
       FROM user_free_spins_campaigns
       WHERE user_id = $1
 ...
 * @access Player
 */
router.post('/:campaignCode/spin', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const { campaignCode } = req.params;
        const userId = req.user.userId || req.user.id;
        console.log(`[CAMPAIGNS] User ${userId} spinning campaign ${campaignCode}`);
        // Get campaign details
        const campaignQuery = await postgres_1.default.query(`SELECT c.*, cg.game_id, cg.total_bet
       FROM campaigns c
       LEFT JOIN campaign_games cg ON c.id = cg.campaign_id
       WHERE c.campaign_code = $1
       LIMIT 1`, [campaignCode]);
        ...
        const userCampaignQuery = await postgres_1.default.query(`SELECT * FROM user_free_spins_campaigns
       WHERE user_id = $1 AND campaign_code = $2`, [userId, campaignCode]);
        ...
        // Update database
        const updateResult = await postgres_1.default.query(`UPDATE user_free_spins_campaigns
       ...
       RETURNING *`, [winAmount, userId, campaignCode]);
