 ...
 */
router.post('/validate-transaction', auth_middleware_1.authenticate, auth_middleware_1.adminAuth, async (req, res) => {
    try {
    ...
    `;
        ...
        const transactionResult = await postgres_1.default.query(transactionQuery, transactionParams);
 ...
 *         description: Forbidden
 */
router.get('/list-cancellable-transactions', auth_middleware_1.authenticate, auth_middleware_1.adminAuth, async (req, res) => {
    try {
      ...
      LIMIT $2
    `;
        const result = await postgres_1.default.query(query, [user_id, limit]);
        return res.json({
            success: true,
 ...
 */
router.get('/transaction-details', auth_middleware_1.authenticate, auth_middleware_1.adminAuth, async (req, res) => {
    try {
      ...
      LIMIT 1
    `;
        const result = await postgres_1.default.query(query, [transaction_id]);
