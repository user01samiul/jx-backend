...
exports.JackpotSyncService = void 0;
 ...
 */
class JackpotSyncService {
    static INNOVA_API_HOST = process.env.INNOVA_API_HOST || 'https://ttlive.me';
    static INNOVA_OPERATOR_ID = env_1.env.SUPPLIER_OPERATOR_ID;
    static INNOVA_PLATFORM_ID = process.env.INNOVA_PLATFORM_ID || 'thinkcode';
    /**
     * Fetch current jackpot amounts from Innova API
     */
    static async fetchInnovaJackpots() {
        try {
     ...
     */
    static async syncJackpotAmounts() {
        ...
        try {
            ...
            const localJackpotsResult = await postgres_1.default.query(`
      ...
      `);
            ...
            for (const innovaJackpot of innovaJackpots) {
                try {
                    ...
                    const localJackpot = localJackpots.find(lj => lj.innova_schedule_id === innovaJackpot.id ||
                    ...
                    if (newAmount !== oldAmount) {
                        // Update schedule
                        await postgres_1.default.query('UPDATE jackpot_schedules SET current_amount = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [newAmount, localJackpot.id]);
                        // Update instance if exists
                        if (localJackpot.instance_id) {
                            await postgres_1.default.query('UPDATE jackpot_instances SET current_amount = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [newAmount, localJackpot.instance_id]);
                        }
     ...
     */
    static startPeriodicSync(intervalMinutes = 5) {
...
exports.JackpotSyncService = JackpotSyncService;
