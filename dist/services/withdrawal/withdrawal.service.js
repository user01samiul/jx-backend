...
const payment_integration_service_1 = require("../payment/payment-integration.service");
class WithdrawalService {
    /**
     * Get withdrawal settings from database
     */
    static async getSettings() {
        const result = await postgres_1.default.query('SELECT setting_key, setting_value, data_type FROM withdrawal_settings');
        const settings = {};
     ...
     */
    static async updateSettings(settings, updatedBy) {
        ...
        try {
           ...
           WHERE setting_key = $3`, [String(value), updatedBy, key]);
            }
    ...
    static async createWithdrawalRequest(request) {
        ...
        try {
            ...
            const userResult = await client.query(`SELECT u.id, u.username, u.email, u.status_id, s.name as status_name,
         ...
         WHERE u.id = $1`, [request.user_id]);
            ...
            const depositResult = await client.query(`SELECT COALESCE(SUM(amount), 0) as total_deposits
         FROM transactions
         WHERE user_id = $1 AND type = 'deposit' AND status = 'completed'`, [request.user_id]);
            ...
            const pendingResult = await client.query(`SELECT COUNT(*) as pending_count
         FROM withdrawal_requests
         WHERE user_id = $1 AND status IN ('pending', 'approved', 'processing')`, [request.user_id]);
            ...
            const limitsResult = await client.query(`SELECT
         ...
         FROM withdrawal_requests
         WHERE user_id = $1 AND status NOT IN ('rejected', 'cancelled', 'failed')`, [request.user_id]);
            ...
            const withdrawalResult = await client.query(`INSERT INTO withdrawal_requests (
        ...
        RETURNING id`, [
            ...
            await client.query('UPDATE withdrawal_requests SET transaction_id = $1 WHERE id = $2', [transactionResult.rows[0].id, withdrawal.id]);
            // 15. Deduct balance (lock funds)
            await client.query('UPDATE user_balances SET balance = balance - $1, updated_at = CURRENT_TIMESTAMP WHERE user_id = $2', [request.amount, request.user_id]);
            // 16. Create audit log
     ...
     */
    static async performFraudChecks(client, userId, amount, ipAddress) {
        const checks = {
        ...
        const recentWithdrawals = await client.query(`SELECT COUNT(*) as count
       FROM withdrawal_requests
       WHERE user_id = $1 AND requested_at > NOW() - INTERVAL '1 hour'`, [userId]);
        ...
        const depositVsWithdrawal = await client.query(`SELECT
       ...
       WHERE user_id = $1 AND status = 'completed'`, [userId]);
        ...
        if (ipAddress) {
            const ipCheck = await client.query(`SELECT COUNT(*) as count
         FROM user_sessions
         WHERE user_id = $1 AND ip_address = $2`, [userId, ipAddress]);
        ...
        const suspiciousActivity = await client.query(`SELECT COUNT(*) as count
       FROM withdrawal_requests
       WHERE user_id = $1 AND status IN ('rejected', 'failed')
       AND requested_at > NOW() - INTERVAL '7 days'`, [userId]);
     ...
     */
    static isWithinAutoProcessHours(settings) {
     ...
     * Process withdrawal through payment gateway
     */
    static async processWithdrawal(withdrawalId, clientConnection) {
        const client = clientConnection || await postgres_1.default.connect();
        ...
        try {
            ...
            const withdrawalResult = await client.query(`SELECT wr.*, u.email, u.username
         ...
         WHERE wr.id = $1`, [withdrawalId]);
            ...
            await client.query('UPDATE withdrawal_requests SET status = $1, processed_at = CURRENT_TIMESTAMP WHERE id = $2', ['processing', withdrawalId]);
           ...
           SET status = 'failed',
               gateway_response = $1,
               updated_at = CURRENT_TIMESTAMP
           WHERE id = $2`, [JSON.stringify(paymentResponse), withdrawalId]);
                // Refund balance
                await client.query('UPDATE user_balances SET balance = balance + $1 WHERE user_id = $2', [withdrawal.amount, withdrawal.user_id]);
                // Update transaction status
                await client.query('UPDATE transactions SET status = $1 WHERE id = $2', ['failed', withdrawal.transaction_id]);
     ...
     */
    static async approveWithdrawal(approval) {
        const client = await postgres_1.default.connect();
        try {
            await client.query('BEGIN');
            // Get withdrawal
            const withdrawalResult = await client.query('SELECT * FROM withdrawal_requests WHERE id = $1', [approval.withdrawal_id]);
            if (withdrawalResult.rows.length === 0) {
                throw new Error('Withdrawal not found');
            }
            const withdrawal = withdrawalResult.rows[0];
     ...
     * Reject withdrawal
     */
    static async rejectWithdrawal(rejection) {
        const client = await postgres_1.default.connect();
        try {
            await client.query('BEGIN');
            // Get withdrawal
            const withdrawalResult = await client.query('SELECT * FROM withdrawal_requests WHERE id = $1', [rejection.withdrawal_id]);
            if (withdrawalResult.rows.length === 0) {
                throw new Error('Withdrawal not found');
            }
            const withdrawal = withdrawalResult.rows[0];
         ...
         SET status = 'rejected',
             ...
             rejection_reason = $2,
             updated_at = CURRENT_TIMESTAMP
         WHERE id = $3`, [rejection.rejected_by, rejection.rejection_reason, rejection.withdrawal_id]);
            // Refund balance
            await client.query('UPDATE user_balances SET balance = balance + $1 WHERE user_id = $2', [withdrawal.amount, withdrawal.user_id]);
            // Update transaction
            await client.query('UPDATE transactions SET status = $1 WHERE id = $2', ['cancelled', withdrawal.transaction_id]);
            await this.createAuditLog(client, rejection.withdrawal_id, 'rejected', rejection.rejected_by, 'admin', withdrawal.status, 'rejected', {
    ...
    static async getDashboardStatistics() {
        // Current month stats
        const currentResult = await postgres_1.default.query(`
      SELECT
    ...
    `);
        ...
        const lastMonthResult = await postgres_1.default.query(`
    ...
    `);
        // This month stats
        const thisMonthResult = await postgres_1.default.query(`
      SELECT COUNT(*) as total_payouts
      FROM withdrawal_requests
      WHERE requested_at >= DATE_TRUNC('month', NOW())
    `);
        // Recent payouts (last 10)
        const recentResult = await postgres_1.default.query(`
      SELECT
        wr.id,
        wr.amount,
        wr.status,
    ...
    static async processPendingWithdrawals() {
        ...
        try {
            ...
            const withdrawalsResult = await client.query(`SELECT id FROM withdrawal_requests
