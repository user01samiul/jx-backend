...
class ReportsService {
     ...
     */
    async getAllReports(category, status) {
    ...
    async getReportById(reportId) {
        const result = await postgres_1.default.query('SELECT * FROM custom_reports WHERE id = $1', [reportId]);
    ...
    async createReport(report) {
        ...
        const result = await postgres_1.default.query(`INSERT INTO custom_reports (
    ...
    async updateReport(reportId, updates) {
        ...
        const result = await postgres_1.default.query(`UPDATE custom_reports SET ${fields.join(', ')}, updated_at = CURRENT_TIMESTAMP
    ...
    async deleteReport(reportId) {
        const result = await postgres_1.default.query('DELETE FROM custom_reports WHERE id = $1', [reportId]);
        return result.rowCount > 0;
    }
    /**
     * Execute custom report
     */
    async executeReport(reportId, executedBy, params) {
        const startTime = Date.now();
        const client = await postgres_1.default.connect();
        try {
            await client.query('BEGIN');
            // Get report definition
            const reportResult = await client.query('SELECT * FROM custom_reports WHERE id = $1 AND status = $2', [reportId, 'ACTIVE']);
            if (reportResult.rows.length === 0) {
                throw new Error('Report not found or inactive');
            }
            const report = reportResult.rows[0];
            // Create execution record
            const executionResult = await client.query(`INSERT INTO report_executions (report_id, executed_by, parameters, status)
         VALUES ($1, $2, $3, 'RUNNING')
         RETURNING id`, [reportId, executedBy, params ? JSON.stringify(params) : null]);
            ...
            await client.query(`UPDATE report_executions
         SET status = 'COMPLETED', row_count = $1, execution_time_ms = $2, completed_at = CURRENT_TIMESTAMP
         WHERE id = $3`, [queryResult.rowCount, executionTime, executionId]);
         ...
         ORDER BY created_at DESC LIMIT 1`, [error.message, reportId, executedBy]);
            throw error;
        }
        finally {
            client.release();
     ...
     */
    async getReportExecutions(reportId, limit = 50) {
        const result = await postgres_1.default.query(`SELECT re.*, u.username as executed_by_username
    ...
    async getAllExecutions(limit = 100) {
        const result = await postgres_1.default.query(`SELECT re.*, cr.name as report_name, u.username as executed_by_username
       FROM report_executions re
     ...
     */
    async createFromTemplate(templateIndex, createdBy) {
