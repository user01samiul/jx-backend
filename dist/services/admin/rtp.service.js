...
exports.getRTPTrendsService = exports.getRTPComplianceService = exports.generateRTPReportService = exports.bulkUpdateRTPService = exports.updateRTPSettingsService = exports.getRTPAnalyticsService = exports.updateRTPSettingsInDatabase = exports.getRTPSettingsFromDatabase = void 0;
 ...
 */
const getRTPSettingsFromDatabase = async () => {
    try {
      ...
      LIMIT 1
    `;
        const settingsResult = await postgres_1.default.query(settingsQuery);
...
exports.getRTPSettingsFromDatabase = getRTPSettingsFromDatabase;
/**
 * Update RTP Settings in rtp_settings table
 */
const updateRTPSettingsInDatabase = async (rtpData) => {
    try {
        console.log('[RTP_SERVICE] Updating RTP settings:', JSON.stringify(rtpData, null, 2));
        // Extract all possible fields from the request
        const { global, provider_settings, category_settings, settings_metadata, 
        // Legacy fields for backward compatibility
        default_rtp, global_rtp, target_profit, rtp_ranges, rtp_categories } = rtpData;
    ...
    `;
        ...
        const result = await postgres_1.default.query(updateQuery, values);
        ...
        RETURNING *
      `;
            const insertResult = await postgres_1.default.query(insertQuery, values);
...
exports.updateRTPSettingsInDatabase = updateRTPSettingsInDatabase;
const getRTPAnalyticsService = async (filters = {}) => {
    ...
    ${whereClause}
  `;
    const betsStatsResult = await postgres_1.default.query(betsStatsQuery, values);
  ...
  `;
    ...
    if (filters.startDate)
        ptValues.push(filters.startDate);
    if (filters.endDate)
        ptValues.push(filters.endDate);
    const profitTrackingResult = await postgres_1.default.query(profitTrackingQuery, ptValues);
    ...
    LIMIT 1
  `;
    const rtpSettingsResult = await postgres_1.default.query(rtpSettingsQuery);
    ...
    ${whereClause}
  `;
    const rtpStatsResult = await postgres_1.default.query(rtpStatsQuery, values);
  ...
  `;
    const rtpByCategoryResult = await postgres_1.default.query(rtpByCategoryQuery, values);
    // Get RTP by provider
    ...
    ORDER BY avg_rtp DESC
  `;
    const rtpByProviderResult = await postgres_1.default.query(rtpByProviderQuery, values);
    ...
    LIMIT 10
  `;
    const recentChangesResult = await postgres_1.default.query(recentChangesQuery, values);
    return {
...
exports.getRTPAnalyticsService = getRTPAnalyticsService;
const updateRTPSettingsService = async (rtpData) => {
  ...
  `;
    const result = await postgres_1.default.query(updateQuery, [JSON.stringify(default_rtp)]);
    return {
        default_rtp,
        rtp_ranges,
        rtp_categories,
...
exports.updateRTPSettingsService = updateRTPSettingsService;
const bulkUpdateRTPService = async (updateData) => {
...
exports.bulkUpdateRTPService = bulkUpdateRTPService;
const generateRTPReportService = async (reportData) => {
  ...
  `;
    const result = await postgres_1.default.query(reportQuery, values);
    if (format === 'csv') {
        // Generate CSV format
        const headers = [
...
exports.generateRTPReportService = generateRTPReportService;
// =====================================================
// RTP COMPLIANCE MONITORING
// =====================================================
const getRTPComplianceService = async () => {
  ...
  `;
    const result = await postgres_1.default.query(complianceQuery);
...
exports.getRTPComplianceService = getRTPComplianceService;
// =====================================================
// RTP TREND ANALYSIS
// =====================================================
const getRTPTrendsService = async (days = 30) => {
    const trendQuery = `
    SELECT 
    ...
    ORDER BY date DESC
  `;
    const result = await postgres_1.default.query(trendQuery);
    return {
        period_days: days,
        trends: result.rows,
        summary: {
...
exports.getRTPTrendsService = getRTPTrendsService;
