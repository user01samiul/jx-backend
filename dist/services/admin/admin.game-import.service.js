...
class AdminGameImportService {
    ...
    async updateProviderConfigById(id, data) {
        const updateFields = [];
        const values = [];
        let paramCount = 1;
    ...
    async deleteProviderConfigById(id) {
        ...
        try {
            ...
            const providerResult = await client.query('SELECT provider_name FROM game_provider_configs WHERE id = $1', [id]);
      ...
      `, [providerName]);
            ...
            const deleteResult = await client.query('DELETE FROM game_provider_configs WHERE id = $1 RETURNING *', [id]);
