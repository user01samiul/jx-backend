...
class BalanceService {
     ...
     */
    static async getUserBalanceWithSync(userId) {
     ...
     */
    static async getCategoryBalanceWithSync(userId, category) {
    ...
    static async addBalance(userId, amount, description, metadata) {
        ...
        try {
            ...
            // Use proper decimal arithmetic to avoid floating-point precision issues
            const balanceAfter = currency_utils_1.CurrencyUtils.add(balanceBefore, amount);
    ...
    static async deductBalance(userId, amount, description, metadata) {
        ...
        try {
            ...
            const balanceBefore = currentBalance.balance;
            // Use proper decimal arithmetic to avoid floating-point precision issues
            const balanceAfter = currency_utils_1.CurrencyUtils.subtract(balanceBefore, amount);
     ...
     */
    static async processDeposit(userId, amount, description, external_reference, metadata) {
        const client = await postgres_1.default.connect();
        try {
            await client.query('BEGIN');
            // Get current balance
     ...
     */
    static async distributeDepositToCategories(userId, depositAmount, currency, client) {
        try {
     ...
     */
    static async getBalanceBreakdown(userId) {
        const client = await postgres_1.default.connect();
        try {
            const balance = await this.calculateRealTimeBalance(userId);
