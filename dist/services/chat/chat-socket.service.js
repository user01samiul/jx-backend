...
class ChatSocketService {
    ...
    async handleAgentStatusChange(socket, status) {
        try {
            ...
            await postgres_1.default.query(`INSERT INTO chat_agents (user_id, status, last_active_at)
         VALUES ($1, $2, CURRENT_TIMESTAMP)
         ON CONFLICT (user_id)
         DO UPDATE SET status = $2, last_active_at = CURRENT_TIMESTAMP`, [socket.userId, status]);
    ...
    async handleStartChat(socket, data) {
        try {
            ...
            try {
                ...
                // Create chat session
                const sessionResult = await client.query(`INSERT INTO chat_sessions (user_id, status, priority, subject)
           VALUES ($1, 'waiting', $2, $3)
           RETURNING id, started_at`, [socket.userId, data.priority || 'normal', data.subject || null]);
                const sessionId = sessionResult.rows[0].id;
                // Calculate priority score (VIP users get higher priority)
                const userResult = await client.query(`SELECT uvs.current_tier_id
           FROM users u
           LEFT JOIN user_vip_status uvs ON u.id = uvs.user_id
           WHERE u.id = $1`, [socket.userId]);
                const vipTierId = userResult.rows[0]?.current_tier_id;
                const priorityScore = vipTierId ? 100 + vipTierId * 10 : 50;
                // Add to queue
                await client.query(`INSERT INTO chat_queue (session_id, priority_score)
           VALUES ($1, $2)`, [sessionId, priorityScore]);
    ...
    async handleRejoinSession(socket, sessionId) {
        try {
            ...
            const sessionCheck = await postgres_1.default.query(`SELECT id, user_id, assigned_agent_id, status
         FROM chat_sessions
         WHERE id = $1 AND (user_id = $2 OR assigned_agent_id = $2)`, [sessionId, socket.userId]);
    ...
    async handleAcceptChat(socket, sessionId) {
        try {
            if (socket.role !== 'Admin' && socket.role !== 'Support') {
            ...
            try {
                ...
                const sessionCheck = await client.query(`SELECT status, user_id FROM chat_sessions WHERE id = $1`, [sessionId]);
                if (sessionCheck.rows.length === 0) {
    ...
    async handleSendMessage(socket, data) {
        try {
            ...
            const sessionCheck = await postgres_1.default.query(`SELECT user_id, assigned_agent_id, status FROM chat_sessions WHERE id = $1`, [data.sessionId]);
            if (sessionCheck.rows.length === 0) {
            ...
            // Insert message
            const messageResult = await postgres_1.default.query(`INSERT INTO chat_session_messages (session_id, sender_id, sender_type, content, message_type)
         VALUES ($1, $2, $3, $4, $5)
         RETURNING id, created_at`, [data.sessionId, socket.userId, senderType, data.content, data.messageType || 'text']);
            const message = {
            ...
            await postgres_1.default.query(`UPDATE chat_sessions SET total_messages = total_messages + 1 WHERE id = $1`, [data.sessionId]);
            // If this is first agent response, record first_response_time
            if (isAgent) {
                await postgres_1.default.query(`UPDATE chat_sessions
           SET first_response_time = COALESCE(first_response_time, EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - started_at))::INTEGER)
           WHERE id = $1`, [data.sessionId]);
            }
    ...
    async handleTypingIndicator(socket, data) {
        try {
            if (data.isTyping) {
                await postgres_1.default.query(`INSERT INTO chat_typing_indicators (session_id, user_id, is_typing)
           VALUES ($1, $2, true)
           ON CONFLICT (session_id, user_id)
           DO UPDATE SET is_typing = true, started_at = CURRENT_TIMESTAMP, expires_at = CURRENT_TIMESTAMP + INTERVAL '10 seconds'`, [data.sessionId, socket.userId]);
            }
            else {
                await postgres_1.default.query(`DELETE FROM chat_typing_indicators WHERE session_id = $1 AND user_id = $2`, [data.sessionId, socket.userId]);
            }
    ...
    async handleMarkAsRead(socket, data) {
        try {
            await postgres_1.default.query(`UPDATE chat_session_messages
         SET is_read = true, read_at = CURRENT_TIMESTAMP
         WHERE id = ANY($1) AND session_id = $2`, [data.messageIds, data.sessionId]);
    ...
    async handleCloseChat(socket, data) {
        try {
           ...
           SET status = 'closed',
               ...
               closed_by = $1,
               close_reason = $2
           WHERE id = $3`, [socket.userId, data.closeReason || 'user_ended', data.sessionId]);
    ...
    async handleGetHistory(socket, data) {
        try {
            const messagesResult = await postgres_1.default.query(`SELECT m.id, m.sender_id, m.sender_type, m.content, m.message_type, m.is_read, m.created_at,
                u.username as sender_name
         FROM chat_session_messages m
         JOIN users u ON m.sender_id = u.id
         WHERE m.session_id = $1
         ORDER BY m.created_at ASC
         LIMIT $2 OFFSET $3`, [data.sessionId, data.limit || 100, data.offset || 0]);
    ...
    async handleGetActiveSessions(socket) {
        try {
            if (socket.role !== 'Admin' && socket.role !== 'Agent') {
                socket.emit('error', { message: 'Only agents can view active sessions' });
                return;
            }
            const sessionsResult = await postgres_1.default.query(`SELECT cs.id, cs.user_id, cs.status, cs.priority, cs.subject, cs.started_at, cs.total_messages,
                u.username as player_name, u.avatar_url
         FROM chat_sessions cs
         JOIN users u ON cs.user_id = u.id
         WHERE cs.assigned_agent_id = $1 AND cs.status = 'active'
         ORDER BY cs.started_at DESC`, [socket.userId]);
    ...
    async handleGetQueueStatus(socket) {
        try {
            const queueResult = await postgres_1.default.query(`SELECT COUNT(*) as total_waiting,
         ...
         WHERE cs.status = 'waiting'`);
            const userQueueResult = await postgres_1.default.query(`SELECT cq.position, cq.estimated_wait_time
         FROM chat_queue cq
         JOIN chat_sessions cs ON cq.session_id = cs.id
         WHERE cs.user_id = $1 AND cs.status = 'waiting'
         LIMIT 1`, [socket.userId]);
