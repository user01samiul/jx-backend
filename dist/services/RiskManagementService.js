...
class RiskManagementService {
    ...
    async getRuleById(ruleId) {
        const result = await postgres_1.default.query('SELECT * FROM risk_rules WHERE id = $1', [ruleId]);
    ...
    async createRule(rule) {
        const result = await postgres_1.default.query(`INSERT INTO risk_rules (
    ...
    async updateRule(ruleId, updates) {
        ...
        values.push(ruleId);
        const result = await postgres_1.default.query(`UPDATE risk_rules SET ${fields.join(', ')}, updated_at = CURRENT_TIMESTAMP
    ...
    async deleteRule(ruleId) {
        const result = await postgres_1.default.query('DELETE FROM risk_rules WHERE id = $1', [ruleId]);
    ...
    async getUserRiskData(userId) {
        const user = await postgres_1.default.query('SELECT * FROM users WHERE id = $1', [userId]);
        // Get recent transactions
        const deposits = await postgres_1.default.query(`SELECT COUNT(*) as count, SUM(amount) as total
       FROM transactions
       WHERE user_id = $1 AND type = 'deposit' AND created_at > NOW() - INTERVAL '30 days'`, [userId]);
        const withdrawals = await postgres_1.default.query(`SELECT COUNT(*) as count, SUM(amount) as total
       FROM transactions
       WHERE user_id = $1 AND type = 'withdrawal' AND created_at > NOW() - INTERVAL '30 days'`, [userId]);
        return {
            user: user.rows[0],
     ...
     */
    async checkDepositPattern(userId, condition) {
        ...
        const result = await postgres_1.default.query(`SELECT COUNT(*) as count, SUM(amount) as total
       FROM transactions
       WHERE user_id = $1 AND type = 'deposit'
       AND created_at > NOW() - INTERVAL '${timeframe} hours'`, [userId]);
     ...
     */
    async checkWithdrawalPattern(userId, condition) {
        const timeframe = condition.timeframe_hours || 24;
     ...
     */
    async checkBettingPattern(userId, condition) {
        const timeframe = condition.timeframe_hours || 24;
     ...
     * Check win rate
     */
    async checkWinRate(userId, condition) {
     ...
     * Check bonus abuse
     */
    async checkBonusAbuse(userId, condition) {
       ...
       AND created_at > NOW() - INTERVAL '${timeframe} days'`, [userId]);
     ...
     * Log risk event
     */
    async logRiskEvent(userId, ruleId, eventType, riskLevel, details) {
        const result = await postgres_1.default.query(`INSERT INTO risk_events (user_id, rule_id, event_type, risk_level, details, resolved)
       VALUES ($1, $2, $3, $4, $5, false)
       RETURNING *`, [userId, ruleId, eventType, riskLevel, JSON.stringify(details)]);
     ...
     * Execute risk action
     */
    async executeRiskAction(userId, rule) {
        ...
        switch (rule.action) {
            ...
            case 'FLAG':
                ...
                await postgres_1.default.query("UPDATE users SET status = 'flagged', updated_at = CURRENT_TIMESTAMP WHERE id = $1", [userId]);
                break;
            case 'LIMIT':
                // Apply limits (deposit limit, bet limit, etc.)
                const limitAmount = actionConfig.limit_amount || 100;
                await postgres_1.default.query(`UPDATE users SET deposit_limit = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2`, [limitAmount, userId]);
                break;
            case 'BLOCK':
                // Block user account
                await postgres_1.default.query("UPDATE users SET is_active = false, status = 'blocked', updated_at = CURRENT_TIMESTAMP WHERE id = $1", [userId]);
                break;
            case 'REVIEW':
                // Mark for manual review
                await postgres_1.default.query("UPDATE users SET requires_review = true, updated_at = CURRENT_TIMESTAMP WHERE id = $1", [userId]);
                break;
        }
    }
    /**
     * Update player risk score
     */
    async updatePlayerRiskScore(userId, riskLevel) {
        const riskPoints = {
            'LOW': 1,
            'MEDIUM': 3,
            'HIGH': 5,
        ...
        await postgres_1.default.query(`INSERT INTO player_risk_scores (user_id, risk_score, last_updated)
       VALUES ($1, $2, CURRENT_TIMESTAMP)
       ON CONFLICT (user_id)
       DO UPDATE SET risk_score = player_risk_scores.risk_score + $2, last_updated = CURRENT_TIMESTAMP`, [userId, points]);
    }
    /**
     * Get risk events for user
     */
    async getUserRiskEvents(userId, resolved, limit = 50) {
        let query = `
     ...
     */
    async getPlayerRiskScore(userId) {
        const result = await postgres_1.default.query('SELECT * FROM player_risk_scores WHERE user_id = $1', [userId]);
        return result.rows[0];
    }
    /**
     * Resolve risk event
     */
    async resolveRiskEvent(eventId, resolution) {
        const result = await postgres_1.default.query(`UPDATE risk_events
       SET resolved = true, action_taken = $1, updated_at = CURRENT_TIMESTAMP
       WHERE id = $2
       RETURNING *`, [resolution, eventId]);
        return result.rows[0];
    }
    /**
     * Get high-risk players (Admin)
     */
    async getHighRiskPlayers(minScore = 10, limit = 100) {
        const result = await postgres_1.default.query(`SELECT prs.*, u.username, u.email, u.status
...
exports.default = new RiskManagementService();
