...
exports.JxOriginalsGameService = void 0;
...
class JxOriginalsGameService {
    /**
     * Main entry point for game commands
     */
    static async handleGameCommand(command) {
        ...
        switch (commandId) {
            ...
            case '2': // Spin
                return await this.handleSpin(command, commandParts);
            ...
            default:
                throw new apiError_1.ApiError(`Unknown game command: ${commandId}`, 400);
        }
    }
    /**
     ...
     */
    static async handleInitGame(userId, gameCode) {
        // Get user balance
        ...
        const lastEventResult = await postgres_1.default.query(`SELECT metadata FROM user_activity_logs
     ...
     * Handle spin command
     */
    static async handleSpin(command, commandParts) {
        ...
        // Log the spin
        await postgres_1.default.query(`INSERT INTO user_activity_logs (user_id, action, category, description, metadata)
     ...
     */
    static generateReels(gameCode, mode = 'normal') {
        const reelStrips = REEL_STRIPS[gameCode];
        if (!reelStrips) {
            throw new apiError_1.ApiError(`No reel strips found for game: ${gameCode}`, 500);
        }
        const result = {
            ways: 1
        };
     ...
     */
    static calculateWins(reels, betline, betlineRaw, gameCode) {
        const paytable = PAYTABLES[gameCode];
        if (!paytable) {
            throw new apiError_1.ApiError(`No paytable found for game: ${gameCode}`, 500);
        }
        let totalWin = 0;
        ...
        for (let sym = 1; sym <= 10; sym++) {
            ...
            for (let reelNum = 1; reelNum <= 6; reelNum++) {
                ...
                if (!found)
                    break; // Stop if reel has no matching symbols
            }
     ...
     */
    static formatReelsForResponse(reels) {
        const result = [];
     ...
     * Handle bonus respin
     */
    static async handleBonusRespin(userId, gameCode) {
     ...
     * Get last response
     */
    static async handleGetLastResponse(userId, gameCode) {
...
exports.JxOriginalsGameService = JxOriginalsGameService;
exports.default = JxOriginalsGameService;
