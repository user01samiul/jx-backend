...
class BonusPlanService {
    ...
    static async getPlanAnalytics(planId) {
        ...
        try {
          ...
          COUNT(CASE WHEN status IN ('active', 'wagering') THEN 1 END) as currently_active,
        ...
        WHERE bonus_plan_id = $1`, [planId]);
            ...
            const completionRate = stats.total_granted > 0
                ...
                : 0;
            // Calculate ROI (simplified: wagering generated / bonus cost)
            const totalBonusCost = parseFloat(stats.total_bonus_cost) || 0;
            const totalWagered = parseFloat(stats.total_wagered) || 0;
            const roi = totalBonusCost > 0 ? totalWagered / totalBonusCost : 0;
            return {
                bonus_plan_id: planId,
                ...
                total_bonus_cost: totalBonusCost,
