...
exports.EnhancedAffiliateService = void 0;
const postgres_1 = __importDefault(require("../../db/postgres"));
class EnhancedAffiliateService {
    /**
     * Create enhanced affiliate profile with MLM structure
     */
    static async createEnhancedAffiliateProfile(userId, profileData, uplineReferralCode) {
        ...
        try {
            ...
            if (uplineReferralCode) {
                const uplineResult = await client.query('SELECT user_id, level FROM affiliate_profiles WHERE referral_code = $1 AND is_active = true', [uplineReferralCode]);
    ...
    static async calculateBetRevenueCommission(affiliateId, referredUserId, periodStart, periodEnd) {
        ...
        try {
            ...
            const betResult = await client.query(`SELECT 
        ...
        WHERE user_id = $1 
          AND placed_at >= $2 
          AND placed_at <= $3`, [referredUserId, periodStart, periodEnd]);
     ...
     */
    static async calculateMLMCommissions(baseAffiliateId, referredUserId, transactionId, amount, commissionType) {
        const client = await postgres_1.default.connect();
        try {
            const commissions = [];
          ...
          RETURNING *`, [
           ...
           WHERE user_id = $2`, [commissionAmount, levelAffiliateId]);
                // Get next level upline
                const uplineResult = await client.query('SELECT upline_id FROM affiliate_profiles WHERE user_id = $1', [levelAffiliateId]);
                if (uplineResult.rows.length > 0) {
     ...
     */
    static async getMLMStructure(affiliateId) {
      ...
      WHERE user_id = $1`, [affiliateId]);
        ...
        const downlineResult = await postgres_1.default.query('SELECT user_id FROM affiliate_profiles WHERE upline_id = $1', [affiliateId]);
        const downlineIds = downlineResult.rows.map(row => row.user_id);
        // Get direct and indirect referrals
        const directReferrals = await postgres_1.default.query('SELECT COUNT(*) as count FROM affiliate_relationships WHERE affiliate_id = $1', [affiliateId]);
        const indirectReferrals = await postgres_1.default.query(`SELECT COUNT(*) as count 
       FROM affiliate_relationships ar
       JOIN affiliate_profiles ap ON ar.referred_user_id = ap.user_id
       WHERE ap.upline_id = $1`, [affiliateId]);
        return {
     ...
     */
    static async getEnhancedAffiliateDashboard(userId) {
        const client = await postgres_1.default.connect();
        try {
            // Get basic affiliate profile
            const profileResult = await client.query('SELECT * FROM affiliate_profiles WHERE user_id = $1', [userId]);
            ...
            const commissionStats = await client.query(`SELECT 
        ...
        WHERE affiliate_id = $1`, [userId]);
            // Get recent referrals
            const recentReferrals = await client.query(`SELECT 
          ar.id,
          u.username,
          u.email,
        ...
        LIMIT 10`, [userId]);
            ...
            const recentCommissions = await client.query(`SELECT 
          ac.id,
        ...
        LIMIT 10`, [userId]);
            ...
            const monthlyStats = await client.query(`SELECT 
        ...
        WHERE ar.affiliate_id = $1 
          AND ar.created_at >= DATE_TRUNC('month', CURRENT_DATE)`, [userId]);
            return {
                profile,
     ...
     */
    static async getAdminAffiliateDashboard() {
        const client = await postgres_1.default.connect();
        try {
            // Overall statistics
            const overallStats = await client.query(`SELECT 
        ...
        FROM affiliate_profiles`);
            // Team statistics
            const teamStats = await client.query(`SELECT 
          at.id as team_id,
        ...
        ORDER BY total_commission DESC`);
            ...
            const topAffiliates = await client.query(`SELECT 
          ap.id,
          u.username,
          ap.display_name,
          ap.total_referrals,
        ...
        LIMIT 10`);
            // Recent activities
            const recentActivities = await client.query(`SELECT 
          'commission' as type,
          ac.id,
    ...
    static async getManagerDashboard(managerId) {
        ...
        try {
            ...
            const teamsResult = await client.query(`SELECT 
          at.id,
          at.name,
          at.description,
        ...
        GROUP BY at.id, at.name, at.description, at.team_commission_rate`, [managerId]);
        ...
        ORDER BY ap.total_commission_earned DESC`, [managerId]);
    ...
    static async getAffiliateStats(userId) {
        ...
        WHERE user_id = $1`, [userId]);
            ...
            const monthlyResult = await client.query(`SELECT COALESCE(SUM(commission_amount), 0) as monthly_commission
         FROM affiliate_commissions 
         WHERE affiliate_id = $1 
         AND created_at >= DATE_TRUNC('month', CURRENT_DATE)`, [userId]);
            // Get pending commission
            const pendingResult = await client.query(`SELECT COALESCE(SUM(commission_amount), 0) as pending_commission
         FROM affiliate_commissions 
         WHERE affiliate_id = $1 
         AND status = 'pending'`, [userId]);
            // Get conversion rate
            const conversionResult = await client.query(`SELECT 
     ...
     */
    static async getAffiliateReferrals(userId, page = 1, limit = 10, level) {
        const client = await postgres_1.default.connect();
        try {
     ...
     */
    static async getAffiliateCommissions(userId, page = 1, limit = 10, status, startDate, endDate) {
        const client = await postgres_1.default.connect();
        try {
            const offset = (page - 1) * limit;
    ...
    static async getAffiliateTeam(userId, level = 1) {
        ...
        ORDER BY ar.created_at DESC`, [userId, level]);
            return {
                level,
     ...
     */
    static async generateAffiliateLink(userId, campaignName, targetUrl) {
        const client = await postgres_1.default.connect();
        try {
            // Get affiliate referral code
            const affiliateResult = await client.query('SELECT referral_code FROM affiliate_profiles WHERE user_id = $1 AND is_active = true', [userId]);
            ...
            await client.query(`INSERT INTO affiliate_links (
          affiliate_id, campaign_name, target_url, affiliate_url, is_active
        ) VALUES ($1, $2, $3, $4, true)`, [userId, campaignName, targetUrl, affiliateUrl]);
            return {
     ...
     */
    static async getAllAffiliateProfiles(page = 1, limit = 10, status, search) {
        const client = await postgres_1.default.connect();
        try {
            const offset = (page - 1) * limit;
            ...
            const profilesResult = await client.query(`SELECT 
          ap.id,
          ap.user_id,
          ap.referral_code,
          ap.display_name,
     ...
     */
    static async getAffiliateCommissionSummary(startDate, endDate, affiliateId) {
        const client = await postgres_1.default.connect();
        try {
            let whereClause = 'WHERE 1=1';
            const params = [];
...
exports.EnhancedAffiliateService = EnhancedAffiliateService;
