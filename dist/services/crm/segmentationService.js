...
exports.AVAILABLE_FILTERS = exports.FilterOperator = exports.FilterCategory = void 0;
...
(function (FilterCategory) {
    FilterCategory["DEMOGRAPHIC"] = "demographic";
    FilterCategory["FINANCIAL"] = "financial";
    FilterCategory["GAMING"] = "gaming";
    FilterCategory["VIP"] = "vip";
    FilterCategory["RISK"] = "risk";
    FilterCategory["ENGAGEMENT"] = "engagement";
...
(function (FilterOperator) {
    ...
    FilterOperator["IN_LAST"] = "in_last";
    FilterOperator["NOT_IN_LAST"] = "not_in_last";
...
class SegmentationService {
    ...
    async getAvailableFilters() {
        ...
        return {
            ...
            categories: Object.keys(exports.AVAILABLE_FILTERS),
            filters: allFilters
        };
    }
    /**
     ...
     */
    buildCondition(filter, startParam) {
        ...
        switch (operator) {
            ...
            case FilterOperator.IN:
                const inPlaceholders = value.map((_, i) => `$${startParam + i}`).join(', ');
                sql = `${fieldMap} IN (${inPlaceholders})`;
                params.push(...value);
                break;
            case FilterOperator.NOT_IN:
                const notInPlaceholders = value.map((_, i) => `$${startParam + i}`).join(', ');
     ...
     */
    async applySegment(filters, limit = 10000) {
    ...
    `;
        ...
        const [players, countResult] = await Promise.all([
            postgres_1.default.query(query, params),
            postgres_1.default.query(countQuery, params.slice(0, -2))
        ]);
        return {
     ...
     */
    async updateSegmentPlayerCount(segmentId, playerIds) {
        const query = `
      UPDATE advanced_segments
      SET player_count = $1, last_refreshed_at = NOW(), updated_at = NOW()
      WHERE id = $2
    `;
        await postgres_1.default.query(query, [playerIds.length, segmentId]);
    }
}
exports.default = new SegmentationService();
