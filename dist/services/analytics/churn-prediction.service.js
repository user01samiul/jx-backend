...
class ChurnPredictionService {
     ...
     */
    static async calculateChurnScore(userId) {
        ...
        // Get user details
        const userQuery = await postgres_1.default.query('SELECT username, email FROM users WHERE id = $1', [userId]);
     ...
     */
    static async getChurnFactors(userId) {
        const query = `
     ...
     */
    static computeChurnScore(factors) {
        let score = 0;
        // Days inactive (0-35 points)
        if (factors.days_inactive > 30)
            score += 35;
        else if (factors.days_inactive > 14)
            score += 25;
        else if (factors.days_inactive > 7)
            score += 15;
        else if (factors.days_inactive > 3)
            score += 5;
        ...
        if (factors.loss_streak >= 5)
            score += 15;
        else if (factors.loss_streak >= 3)
            score += 10;
        else if (factors.loss_streak >= 2)
            score += 5;
     ...
     */
    static identifyRiskFactors(factors) {
        const risks = [];
     ...
     */
    static recommendActions(riskLevel, riskFactors) {
        const actions = [];
        switch (riskLevel) {
            case 'critical':
     ...
     */
    static async calculateAllPredictions() {
        // Get all active users
        const usersQuery = await postgres_1.default.query(`
     ...
     * Get high-risk users
     */
    static async getHighRiskUsers(limit = 100) {
        const query = `
      SELECT
        cp.user_id,
        u.username,
      ...
      LIMIT $1
    `;
        const result = await postgres_1.default.query(query, [limit]);
        return result.rows;
    }
    /**
     ...
     */
    static async runChurnPredictionWorkflow() {
