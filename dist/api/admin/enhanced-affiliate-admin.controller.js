...
exports.adminUpdateSystemSettings = exports.adminGetSystemSettings = exports.adminGetMLMStructure = exports.adminProcessCommissionPayout = exports.adminGetAffiliateAnalytics = exports.adminUpdateAffiliateSettings = exports.adminCreateAffiliate = exports.getAdminAffiliateSystemOverview = void 0;
...
// =====================================================
const getAdminAffiliateSystemOverview = async (req, res) => {
    try {
...
exports.getAdminAffiliateSystemOverview = getAdminAffiliateSystemOverview;
const adminCreateAffiliate = async (req, res) => {
    try {
        ...
        try {
            ...
            const existingProfile = await client.query('SELECT id FROM affiliate_profiles WHERE user_id = $1', [userId]);
            ...
            const profile = await enhanced_affiliate_service_1.EnhancedAffiliateService.createEnhancedAffiliateProfile(userId, {
                ...profileData,
           ...
           VALUES ($1, $2) 
           ON CONFLICT (user_id, role_id) DO NOTHING`, [userId, roleResult.rows[0].id]);
            }
...
exports.adminCreateAffiliate = adminCreateAffiliate;
const adminUpdateAffiliateSettings = async (req, res) => {
    try {
...
exports.adminUpdateAffiliateSettings = adminUpdateAffiliateSettings;
const adminGetAffiliateAnalytics = async (req, res) => {
    try {
        const { affiliateId } = req.params;
        ...
        try {
            ...
            const profileResult = await client.query(`SELECT ap.*, u.username, u.email
         FROM affiliate_profiles ap
         JOIN users u ON ap.user_id = u.id
         WHERE ap.id = $1`, [affiliateId]);
            ...
            const commissionAnalytics = await client.query(`SELECT 
         ...
         ORDER BY date DESC`, [profile.user_id]);
            // Get referral analytics
            const referralAnalytics = await client.query(`SELECT 
         ...
         ORDER BY date DESC`, [profile.user_id]);
            ...
            const mlmAnalytics = await client.query(`SELECT 
           level,
         ...
         WHERE upline_id = $1
         GROUP BY level
         ORDER BY level`, [profile.user_id]);
            // Get performance metrics
            const performanceMetrics = await client.query(`SELECT 
         ...
         FROM affiliate_relationships ar
         WHERE ar.affiliate_id = $1`, [profile.user_id]);
            res.json({
                success: true,
                data: {
                    profile,
...
exports.adminGetAffiliateAnalytics = adminGetAffiliateAnalytics;
const adminProcessCommissionPayout = async (req, res) => {
    try {
        const { affiliateId } = req.params;
        ...
        try {
            ...
            const totalAmountResult = await client.query(`SELECT COALESCE(SUM(commission_amount), 0) as total_amount
         FROM affiliate_commissions 
         WHERE id = ANY($1) AND status = 'pending'`, [commissionIds]);
...
exports.adminProcessCommissionPayout = adminProcessCommissionPayout;
const adminGetMLMStructure = async (req, res) => {
    try {
        const { affiliateId } = req.params;
        ...
        try {
            ...
            const mlmTreeResult = await client.query(`SELECT * FROM get_mlm_tree($1, $2)`, [affiliateId, parseInt(maxDepth)]);
            // Get upline chain
            const uplineChainResult = await client.query(`WITH RECURSIVE upline_chain AS (
        ...
        JOIN users u ON ap.user_id = u.id
        ORDER BY uc.level DESC`, [affiliateId]);
            res.json({
                success: true,
                data: {
...
exports.adminGetMLMStructure = adminGetMLMStructure;
const adminGetSystemSettings = async (req, res) => {
    try {
        const client = await postgres_1.default.connect();
        try {
            ...
            // Get team settings
            const teamSettings = await client.query(`SELECT 
          at.id,
          at.name,
        ...
        ORDER BY at.name`);
            ...
            const managerPermissions = await client.query(`SELECT 
          mp.*,
...
exports.adminGetSystemSettings = adminGetSystemSettings;
const adminUpdateSystemSettings = async (req, res) => {
    try {
        ...
        try {
            ...
            if (teamSettings) {
                for (const team of teamSettings) {
                    await client.query(`UPDATE affiliate_teams 
             SET team_commission_rate = $1, team_goals = $2
             WHERE id = $3`, [team.team_commission_rate, team.team_goals, team.id]);
                }
            }
            // Update manager permissions if provided
            if (managerPermissions) {
                for (const permission of managerPermissions) {
                    await client.query(`UPDATE manager_permissions 
             SET 
...
exports.adminUpdateSystemSettings = adminUpdateSystemSettings;
