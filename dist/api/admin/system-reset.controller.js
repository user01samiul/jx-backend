...
exports.getResetStats = exports.resetSystem = void 0;
...
const resetSystem = async (req, res, next) => {
    ...
    try {
        ...
        const { user_id, reset_type } = req.body;
        // Validate request
        ...
        try {
            ...
            const adminCheck = await client.query(`SELECT r.name as role FROM users u 
         JOIN user_roles ur ON u.id = ur.user_id 
         JOIN roles r ON ur.role_id = r.id 
         WHERE u.id = $1`, [adminUserId]);
            ...
            if (reset_type === 'user' && user_id) {
                const userExists = await client.query('SELECT id, username FROM users WHERE id = $1', [user_id]);
 ...
 */
async function performSystemReset(client, resetType, userId) {
    const resetDetails = {
        deleted_bets: 0,
...
exports.getResetStats = getResetStats;
