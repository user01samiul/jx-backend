...
class AffiliateBalanceService {
    ...
    static async getBalanceSummary(userId) {
        const result = await postgres_1.default.query('SELECT * FROM get_affiliate_balance_summary($1)', [userId]);
     ...
     */
    static async getBalanceHistory(userId, filters) {
        ...
        // Get total count
        const countResult = await postgres_1.default.query(`SELECT COUNT(*) as total FROM affiliate_balance_transactions WHERE ${whereClause}`, params);
     ...
     */
    static async processRedemption(userId, amount, notes) {
        const client = await postgres_1.default.connect();
        try {
            await client.query('BEGIN');
            // Get current affiliate balance
            const balanceResult = await client.query('SELECT affiliate_balance FROM user_balances WHERE user_id = $1 FOR UPDATE', [userId]);
            ...
            const settingsResult = await client.query("SELECT setting_value FROM affiliate_settings WHERE setting_key = 'redemption_settings'");
            ...
            if (amount < minRedemption) {
                throw new apiError_1.ApiError(`Minimum redemption amount is ${minRedemption}`, 400);
            }
         ...
         WHERE user_id = $2`, [amount, userId]);
            ...
            const redemptionResult = await client.query(`INSERT INTO affiliate_redemptions (
        ...
        RETURNING id`, [userId, amount, instantAmount, lockedAmount, unlockDate, notes]);
            ...
            await client.query(`INSERT INTO affiliate_balance_transactions (
     ...
     */
    static async approveRedemption(redemptionId, adminId, adminNotes) {
        const client = await postgres_1.default.connect();
        try {
            await client.query('BEGIN');
            // Get redemption details
            const redemptionResult = await client.query('SELECT * FROM affiliate_redemptions WHERE id = $1 FOR UPDATE', [redemptionId]);
            ...
            if (redemption.instant_status !== 'pending') {
                throw new apiError_1.ApiError(`Redemption already ${redemption.instant_status}`, 400);
            }
         ...
         WHERE user_id = $2`, [instantAmount, userId]);
            // Create transaction in main transactions table for INSTANT amount only
            const mainTxResult = await client.query(`INSERT INTO transactions (
        ...
        RETURNING id`, [
            ...
            await client.query(`UPDATE affiliate_redemptions
     ...
     */
    static async processLockedReleases() {
        let processed = 0;
        let errors = 0;
        try {
            // Find all locked amounts ready for release
            const result = await postgres_1.default.query(`SELECT id, user_id, locked_amount, unlock_date
     ...
     */
    static async releaseLockedAmount(redemptionId) {
        const client = await postgres_1.default.connect();
        try {
            await client.query('BEGIN');
            // Get redemption details
            ...
            if (redemption.locked_status !== 'locked') {
                throw new apiError_1.ApiError('Redemption locked amount already processed', 400);
            }
            const userId = redemption.user_id;
            const lockedAmount = parseFloat(redemption.locked_amount);
            if (lockedAmount <= 0) {
         ...
         SET affiliate_balance_locked = affiliate_balance_locked - $1,
             balance = balance + $1,
             affiliate_total_redeemed = affiliate_total_redeemed + $1
         WHERE user_id = $2`, [lockedAmount, userId]);
            // Create transaction in main transactions table
            const txResult = await client.query(`INSERT INTO transactions (
          user_id, type, amount, currency, status, description,
          metadata
        ) VALUES ($1, 'bonus', $2, 'USD', 'completed',
        ...
        RETURNING id`, [
         ...
         SET locked_status = 'unlocked',
             ...
             unlocked_at = CURRENT_TIMESTAMP
         WHERE id = $2`, [unlockTransactionId, redemptionId]);
     ...
     */
    static async rejectRedemption(redemptionId, adminId, reason, adminNotes) {
        const client = await postgres_1.default.connect();
        try {
            await client.query('BEGIN');
            // Get redemption details
     ...
     */
    static async getRedemptionHistory(userId, filters) {
        const { page = 1, limit = 20 } = filters;
        const offset = (page - 1) * limit;
        // Get total count
        const countResult = await postgres_1.default.query('SELECT COUNT(*) as total FROM affiliate_redemptions WHERE user_id = $1', [userId]);
        const total = parseInt(countResult.rows[0].total);
     ...
     */
    static async getPendingUnlocks(userId) {
        const result = await postgres_1.default.query(`SELECT
        id as redemption_id,
        locked_amount,
        unlock_date,
     ...
     */
    static async unlockPendingRedemptions() {
        const client = await postgres_1.default.connect();
        let unlockedCount = 0;
        try {
            // Get all redemptions ready to unlock
            const result = await client.query(`SELECT * FROM affiliate_redemptions
         WHERE locked_status = 'locked'
           AND unlock_date <= CURRENT_TIMESTAMP
         ORDER BY unlock_date ASC`);
            for (const redemption of result.rows) {
    ...
    static async getAllRedemptions(filters) {
        ...
        // Get total count
        const countResult = await postgres_1.default.query(`SELECT COUNT(*) as total FROM affiliate_redemptions ar ${whereClause}`, params);
        const total = parseInt(countResult.rows[0].total);
     ...
     */
    static async adjustBalance(userId, amount, description, adminId) {
        const client = await postgres_1.default.connect();
        try {
            await client.query('BEGIN');
            // Get current balance
         ...
         WHERE user_id = $2`, [amount, userId]);
            ...
            const result = await client.query(`INSERT INTO affiliate_balance_transactions (
          user_id, transaction_type, amount, balance_before, balance_after,
