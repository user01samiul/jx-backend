 ...
 */
const getPlayerBonusWalletAdmin = async (req, res, next) => {
    try {
        const playerId = parseInt(req.params.playerId);
...
exports.getPlayerBonusWalletAdmin = getPlayerBonusWalletAdmin;
...
const searchPlayer = async (req, res, next) => {
    try {
        ...
        if (!isNaN(playerId)) {
            // Search by ID
            result = await pool.query(`SELECT id, username, email, created_at
         FROM users
         WHERE id = $1
         LIMIT 1`, [playerId]);
        }
 ...
 */
const getAllPlayersWithBonuses = async (req, res, next) => {
    try {
       ...
       LIMIT $${paramCount - 1} OFFSET $${paramCount}`, params);
        // Get total count
        const countResult = await pool.query(`SELECT COUNT(DISTINCT u.id) as total
       FROM users u
       INNER JOIN bonus_instances bi ON bi.player_id = u.id
       ${whereClause}`, params.slice(0, -2) // Remove limit and offset params
        );
        res.status(200).json({
            success: true,
            data: result.rows,
...
exports.getAllPlayersWithBonuses = getAllPlayersWithBonuses;
// =====================================================
 ...
 */
const getBonusAnalyticsOverview = async (req, res, next) => {
    try {
        const pool = require('../../db/postgres').default;
        // Overall statistics
...
exports.getBonusAnalyticsOverview = getBonusAnalyticsOverview;
/**
 * Get bonus time series data for charts
 * Supports daily, weekly, monthly grouping
 */
const getBonusTimeSeries = async (req, res, next) => {
    try {
        const pool = require('../../db/postgres').default;
...
exports.getBonusTimeSeries = getBonusTimeSeries;
/**
 * Get bonus distribution by status (for pie/donut charts)
 */
const getBonusDistributionByStatus = async (req, res, next) => {
    try {
        const pool = require('../../db/postgres').default;
...
exports.getBonusDistributionByStatus = getBonusDistributionByStatus;
/**
 * Get bonus distribution by plan (for bar charts)
 */
const getBonusDistributionByPlan = async (req, res, next) => {
    try {
        const pool = require('../../db/postgres').default;
...
exports.getBonusDistributionByPlan = getBonusDistributionByPlan;
/**
 * Get top performing bonus plans with detailed metrics
 */
const getBonusTopPerformingPlans = async (req, res, next) => {
    try {
        const pool = require('../../db/postgres').default;
...
exports.getBonusTopPerformingPlans = getBonusTopPerformingPlans;
/**
 * Get player engagement metrics
 */
const getBonusPlayerEngagement = async (req, res, next) => {
    try {
        const pool = require('../../db/postgres').default;
        // Player segmentation
...
exports.getBonusPlayerEngagement = getBonusPlayerEngagement;
/**
 * Get financial metrics and ROI analysis
 */
const getBonusFinancialMetrics = async (req, res, next) => {
    try {
        const pool = require('../../db/postgres').default;
...
exports.getBonusFinancialMetrics = getBonusFinancialMetrics;
