...
class BonusEngineService {
        ...
        try {
            ...
            if (remainingBetAmount > 0) {
               ...
               WHERE id = $2`, [usedBonusWallet, bonus.id]);
                        ...
                        const wagerResult = await wagering_engine_service_1.WageringEngineService.processBetWagering(bonus.id, playerId, gameId, usedBonusWallet);
    ...
    static async getCombinedBalance(playerId) {
        ...
        try {
          ...
          COALESCE(SUM(CASE WHEN type = 'cashback' THEN amount ELSE 0 END), 0) +
          ...
          as balance
         FROM transactions
         WHERE user_id = $1 AND status = 'completed'`, [playerId]);
            ...
            return {
                ...
                totalAvailable: mainWallet + bonusWallet.playable_bonus_balance,
