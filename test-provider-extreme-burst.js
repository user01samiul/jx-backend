...
// Simulate extreme provider burst - bets and wins in same burst
async function simulateExtremeProviderBurst() {
  console.log('🔥 Simulating EXTREME Provider Burst (Bets + Wins in Same Burst)...\n');
  ...
  // Create all requests (bets and wins) in the same burst
  ...
  for (let i = 0; i < betAmounts.length; i++) {
    ...
    const transaction_id = 2249000 + i;
  ...
  for (let i = 0; i < winAmounts.length; i++) {
    ...
    const betTransactionId = 2249000 + i;
    const winTransactionId = 2249100 + i;
    const timestamp = new Date().toISOString().replace('T', ' ').substring(0, 19);
  ...
  const dbQuery = `
    ...
    WHERE t.external_reference IN ('${Array.from({length: 5}, (_, i) => 2249000 + i).join("','")}')
    ORDER BY b.id;
  `;
  ...
  console.log('\n🔥 Extreme Provider Burst Simulation Completed!');
  console.log('\n🔍 This test simulates the WORST-CASE scenario:');
  console.log('   - BETS and WINS sent in the SAME burst');
  console.log('   - Maximum race conditions possible');
  console.log('   - Extreme concurrency stress test');
  console.log('   - Real provider edge case behavior');
}

// Run the extreme burst simulation
simulateExtremeProviderBurst().catch(console.error); 