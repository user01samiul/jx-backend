...
async function testCancelOnEnabledGame() {
  try {
    console.log('🧪 Testing CANCEL transaction on enabled game for user 48 (player50)...');
    ...
    console.log('\n📤 Step 2: Placing bet on enabled game (should succeed)...');
    ...
    console.log('\n📤 Step 3: Trying to cancel transaction on enabled game...');
    ...
    if (cancelResponse.data.response.status === 'OK') {
      console.log('✅ Cancel successfully processed');
    ...
    console.log('\n📤 Step 5: Trying to cancel the same transaction again (should fail)...');
    ...
    if (cancelAgainResponse.data.response.status === 'ERROR') {
      console.log('✅ Second cancel correctly rejected (transaction already cancelled)');
    } else {
      console.log('❌ Second cancel should have been rejected');
    }
